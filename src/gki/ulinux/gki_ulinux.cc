...
void GKI_run(void* p_task_id) {
  ...
  for (; GKI_TIMER_TICK_EXIT_COND != *p_run_cond;) {
    do {
      ...
      GKI_timer_update(1);
      /* DLOG_IF(INFO, appl_trace_level >= BT_TRACE_LEVEL_DEBUG) <<
       * StringPrintf( TRACE_LAYER_HCI, TRACE_TYPE_DEBUG, "update: tv_sec: %d,
