...
void* gki_task_entry(void* params) {
  ...
  ALOGE("gki_task task_id=%i terminating", p_pthread_info->task_id);
...
uint8_t GKI_create_task(TASKPTR task_entry, uint8_t task_id, int8_t* taskname,
  ...
  if (task_id >= GKI_MAX_TASKS) {
    ALOGD("Error! task ID > max task allowed");
    return (GKI_FAILURE);
  }
...
/* by default, pthread creates a joinable thread */
  ...
  if (ret != 0) {
    ALOGD("pthread_create failed(%d), %s!", ret, taskname);
    return GKI_FAILURE;
  }
  ...
  if (pthread_getschedparam(gki_cb.os.thread_id[task_id], &policy, &param) ==
    ...
    if (task_id == PBS_SQL_TASK) {
      ALOGD("PBS SQL lowest priority task");
      policy = SCHED_NORMAL;
    } else
#endif
    {
...
void GKI_shutdown(void) {
  ...
  for (task_id = GKI_MAX_TASKS; task_id > 0; task_id--) {
    if (gki_cb.com.OSRdyTbl[task_id - 1] != TASK_DEAD) {
      ...
      if (result < 0) {
        ALOGD("pthread_join() FAILED: result: %d", result);
      }
#endif
      ALOGD("GKI_shutdown(): task %s dead", gki_cb.com.OSTName[task_id]);
...
/*    pthread_mutex_destroy(&thread_delay_mutex);
 pthread_cond_destroy (&thread_delay_cond); */
  ...
  if (gki_cb.os.gki_timer_wake_lock_on) {
    ALOGD("GKI_shutdown :  release_wake_lock(brcm_btld)");
...
void GKI_run(void* p_task_id) {
  ...
  ALOGD("GKI_run(): Start/Stop GKI_timer_update_registered!");
#endif

#ifdef NO_GKI_RUN_RETURN
  ALOGD("GKI_run == NO_GKI_RUN_RETURN");
  ...
  if (pthread_create(&timer_thread_id, &timer_attr, timer_thread, NULL) != 0) {
    ALOGD("GKI_run: pthread_create failed to create timer_thread!");
    return GKI_FAILURE;
  }
#else
 ...
 * block timer main thread till re-armed by  */
    ...
    if (GKI_TIMER_TICK_EXIT_COND != *p_run_cond) {
      ...
      ALOGD("%s exited timer mutex", __func__);
    }
...
uint16_t GKI_wait(uint16_t flag, uint32_t timeout) {
  ...
  ALOGD("GKI_wait %d %x %d", rtask, flag, timeout);
  if (rtask >= GKI_MAX_TASKS) {
    ALOGE("%s() Exiting thread; rtask %d >= %d", __func__, rtask,
          GKI_MAX_TASKS);
  ...
  if (!(gki_cb.com.OSWaitEvt[rtask] & flag)) {
    ...
    if (gki_cb.com.OSRdyTbl[rtask] == TASK_DEAD) {
      ...
      ALOGE("GKI TASK_DEAD received. exit thread %d...", rtask);
   ...
   * cond is met */
  pthread_mutex_unlock(&gki_cb.os.thread_evt_mutex[rtask]);
  ALOGD("GKI_wait %d %x %d %x resumed", rtask, flag, timeout, evt);
...
void GKI_delay(uint32_t timeout) {
  ...
  ALOGD("GKI_delay %d %d", rtask, timeout);
  ...
  */
  ...
  ALOGD("GKI_delay %d %d done", rtask, timeout);
  return;
}
...
uint8_t GKI_send_event(uint8_t task_id, uint16_t event) {
  ALOGD("GKI_send_event %d %x", task_id, event);
  ...
  if (task_id < GKI_MAX_TASKS) {
    ...
    ALOGD("GKI_send_event %d %x done", task_id, event);
    return (GKI_SUCCESS);
  }
  return (GKI_FAILURE);
}
...
uint8_t GKI_isend_event(uint8_t task_id, uint16_t event) {
  ALOGD("GKI_isend_event %d %x", task_id, event);
  ALOGD("GKI_isend_event %d %x done", task_id, event);
...
int8_t* GKI_map_taskname(uint8_t task_id) {
  ALOGD("GKI_map_taskname %d", task_id);

  if (task_id < GKI_MAX_TASKS) {
    ALOGD("GKI_map_taskname %d %s done", task_id, gki_cb.com.OSTName[task_id]);
...
void GKI_exception(uint16_t code, std::string msg) {
  ...
  for (task_id = 0; task_id < GKI_MAX_TASKS; task_id++) {
    ALOGE("TASK ID [%d] task name [%s] state [%d]", task_id,
          gki_cb.com.OSTName[task_id], gki_cb.com.OSRdyTbl[task_id]);
  }

  ALOGE("GKI_exception %d %s", code, msg.c_str());
  ALOGE("********************************************************************");
  ALOGE("* GKI_exception(): %d %s", code, msg.c_str());
  ALOGE("********************************************************************");

  ALOGE("GKI_exception %d %s done", code, msg.c_str());
...
uint8_t GKI_suspend_task(uint8_t task_id) {
  ALOGD("GKI_suspend_task %d - NOT implemented", task_id);

  ALOGD("GKI_suspend_task %d done", task_id);
...
uint8_t GKI_resume_task(uint8_t task_id) {
  ALOGD("GKI_resume_task %d - NOT implemented", task_id);

  ALOGD("GKI_resume_task %d done", task_id);
...
void GKI_exit_task(uint8_t task_id) {
  ...
  ALOGD("GKI_exit_task %d done", task_id);
  return;
}
