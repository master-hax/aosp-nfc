...
void nfa_sys_event(NFC_HDR* p_msg) {
  ...
  ALOGD("NFA got event 0x%04X", p_msg->event);
  ...
  if ((id < NFA_ID_MAX) && (nfa_sys_cb.is_reg[id])) {
    ...
    ALOGW("NFA got unregistered event id %d", id);
  }
...
void nfa_sys_register(uint8_t id, const tNFA_SYS_REG* p_reg) {
  ...
  ALOGD("nfa_sys_register () id=%i, enable_cplt_mask=0x%x", id,
...
void nfa_sys_deregister(uint8_t id) {
  ALOGD("nfa_sys: deregistering subsystem %i", id);
...
void nfa_sys_enable_subsystems(void) {
  ...
  ALOGD("nfa_sys: enabling subsystems");
...
void nfa_sys_disable_subsystems(bool graceful) {
  ...
  ALOGD("nfa_sys: disabling subsystems:%d", graceful);
...
void nfa_sys_notify_nfcc_power_mode(uint8_t nfcc_power_mode) {
  ...
  ALOGD("nfa_sys: notify NFCC power mode(%d) to subsystems", nfcc_power_mode);
