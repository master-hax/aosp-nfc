...
tNFA_HCI_DYN_GATE* nfa_hciu_alloc_gate(uint8_t gate_id,
  ...
  if (gate_id != 0) {
    ...
    if (gate_id > NFA_HCI_LAST_PROP_GATE) {
      ALOGE("nfa_hci_alloc_gate - no free Gate ID: %u  App Handle: 0x%04x",
            gate_id, app_handle);
      return (NULL);
    }
  }
  ...
  for (xx = 0, pg = nfa_hci_cb.cfg.dyn_gates; xx < NFA_HCI_MAX_GATE_CB;
    ...
    if (pg->gate_id == 0) {
      ...
      ALOGD("nfa_hciu_alloc_gate id:%d  app_handle: 0x%04x", gate_id,
            app_handle);
  ...
  ALOGE("nfa_hci_alloc_gate - no CB  Gate ID: %u  App Handle: 0x%04x", gate_id,
        app_handle);
  return (NULL);
}
...
tNFA_STATUS nfa_hciu_send_msg(uint8_t pipe_id, uint8_t type,
  ...
  ALOGD("nfa_hciu_send_msg pipe_id:%d   %s  len:%d", pipe_id,
        nfa_hciu_get_type_inst_names(pipe_id, type, instruction, buff),
        msg_len);
#else
  ALOGD("nfa_hciu_send_msg pipe_id:%d   Type: %u  Inst: %u  len: %d", pipe_id,
  ...
  while ((first_pkt == true) || (msg_len != 0)) {
    ...
    if (p_buf != NULL) {
      ...
      ALOGE("nfa_hciu_send_data_packet no buffers");
...
uint8_t nfa_hciu_get_allocated_gate_list(uint8_t* p_gate_list) {
  ...
  ALOGD("nfa_hciu_get_allocated_gate_list () returns: %u", count);
...
tNFA_HCI_DYN_PIPE* nfa_hciu_alloc_pipe(uint8_t pipe_id) {
  ...
  for (xx = 0, pp = nfa_hci_cb.cfg.dyn_pipes; xx < NFA_HCI_MAX_PIPE_CB;
       xx++, pp++) {
    if (pp->pipe_id == 0) {
      ALOGD("nfa_hciu_alloc_pipe:%d, index:%d", pipe_id, xx);
      pp->pipe_id = pipe_id;
  ...
  ALOGD("nfa_hciu_alloc_pipe:%d, NO free entries !!", pipe_id);
  return (NULL);
}
...
void nfa_hciu_release_gate(uint8_t gate_id) {
  ...
  if (p_gate != NULL) {
    ...
    ALOGW("nfa_hciu_release_gate () ID: %d  NOT FOUND", gate_id);
  }
}
...
tNFA_HCI_RESPONSE nfa_hciu_add_pipe_to_gate(uint8_t pipe_id, uint8_t local_gate,
  ...
  ALOGD("nfa_hciu_add_pipe_to_gate: 0x%02x  NOT FOUND", local_gate);
...
tNFA_HCI_DYN_PIPE* nfa_hciu_find_active_pipe_by_owner(tNFA_HANDLE app_handle) {
  ...
  ALOGD("nfa_hciu_find_pipe_by_owner () app_handle:0x%x", app_handle);
...
tNFA_HCI_DYN_PIPE* nfa_hciu_find_pipe_on_gate(uint8_t gate_id) {
  ...
  ALOGD("nfa_hciu_find_pipe_on_gate () Gate:0x%x", gate_id);
...
tNFA_HCI_DYN_PIPE* nfa_hciu_find_active_pipe_on_gate(uint8_t gate_id) {
  ...
  ALOGD("nfa_hciu_find_active_pipe_on_gate () Gate:0x%x", gate_id);
...
void nfa_hciu_remove_all_pipes_from_host(uint8_t host) {
  ...
  ALOGD("nfa_hciu_remove_all_pipes_from_host (0x%02x)", host);
...
tNFA_STATUS nfa_hciu_send_delete_pipe_cmd(uint8_t pipe) {
  ...
  ALOGD("nfa_hciu_send_delete_pipe_cmd: %d", pipe);

  if (pipe > NFA_HCI_LAST_DYNAMIC_PIPE) {
    ALOGD("ignore pipe: %d", pipe);
    return (NFA_HCI_ANY_E_NOK);
  }
...
tNFA_STATUS nfa_hciu_send_clear_all_pipe_cmd(void) {
  ...
  ALOGD("nfa_hciu_send_clear_all_pipe_cmd");
