...
void nfa_hci_check_api_requests(void) {
  ...
  for (;;) {
    ...
    switch (p_msg->event) {
      ...
      default:
        ALOGE("nfa_hci_check_api_requests ()  Unknown event: 0x%04x",
              p_msg->event);
        break;
    }
  ...
  for (xx = 0; xx < NFA_HCI_MAX_APP_CB; xx++) {
    if ((nfa_hci_cb.cfg.reg_app_names[xx][0] != 0) &&
        !strncmp(p_app_name, &nfa_hci_cb.cfg.reg_app_names[xx][0],
                 strlen(p_app_name))) {
      ALOGD("nfa_hci_api_register (%s)  Reusing: %u", p_app_name, xx);
      break;
    }
  }
  ...
  if (xx != NFA_HCI_MAX_APP_CB) {
    ...
    for (xx = 0; xx < NFA_HCI_MAX_APP_CB; xx++) {
      if (nfa_hci_cb.cfg.reg_app_names[xx][0] == 0) {
        ...
        ALOGD("nfa_hci_api_register (%s)  Allocated: %u", p_app_name, xx);
        break;
      }
    }

    if (xx == NFA_HCI_MAX_APP_CB) {
      ALOGE("nfa_hci_api_register (%s)  NO ENTRIES", p_app_name);
  ...
  if (p_evt_data != NULL) {
    for (xx = 0; xx < NFA_HCI_MAX_APP_CB; xx++) {
      if ((nfa_hci_cb.cfg.reg_app_names[xx][0] != 0) &&
        ...
        ALOGD("nfa_hci_api_deregister (%s) inx: %u",
    ...
    if (xx == NFA_HCI_MAX_APP_CB) {
      ALOGW("nfa_hci_api_deregister () Unknown app: %s",
...
static void nfa_hci_api_alloc_gate(tNFA_HCI_EVENT_DATA* p_evt_data) {
  ...
  if (p_gate) {
    if (!p_gate->gate_owner) {
      ...
      p_gate = NULL;
      ALOGE("nfa_hci_api_alloc_gate (): The Gate (0X%02x) already taken!",
...
static bool nfa_hci_api_get_reg_value(tNFA_HCI_EVENT_DATA* p_evt_data) {
  ...
  if (p_pipe != NULL) {
    ...
    if ((p_gate != NULL) && (nfa_hciu_is_active_host(p_pipe->dest_host)) &&
      ...
      if (p_pipe->pipe_state == NFA_HCI_PIPE_CLOSED) {
        ALOGW("nfa_hci_api_get_reg_value pipe:%d not open",
...
static bool nfa_hci_api_set_reg_value(tNFA_HCI_EVENT_DATA* p_evt_data) {
  ...
  if (p_pipe != NULL) {
    ...
    if ((p_gate != NULL) && (nfa_hciu_is_active_host(p_pipe->dest_host)) &&
      ...
      if (p_pipe->pipe_state == NFA_HCI_PIPE_CLOSED) {
        ALOGW("nfa_hci_api_set_reg_value pipe:%d not open",
  ...
  if ((p_pipe = nfa_hciu_find_pipe_by_pid(p_evt_data->send_cmd.pipe)) != NULL) {
    ...
    if ((nfa_hciu_is_active_host(p_pipe->dest_host)) &&
      ...
      if (p_pipe->pipe_state == NFA_HCI_PIPE_OPENED) {
        ...
        ALOGW("nfa_hci_api_send_cmd pipe:%d not open", p_pipe->pipe_id);
      }
    } else {
      ALOGW(
    ...
    ALOGW("nfa_hci_api_send_cmd pipe:%d not found", p_evt_data->send_cmd.pipe);
  }
  ...
  if ((p_pipe = nfa_hciu_find_pipe_by_pid(p_evt_data->send_rsp.pipe)) != NULL) {
    ...
    if ((nfa_hciu_is_active_host(p_pipe->dest_host)) &&
      ...
      if (p_pipe->pipe_state == NFA_HCI_PIPE_OPENED) {
        ...
        ALOGW("nfa_hci_api_send_rsp pipe:%d not open", p_pipe->pipe_id);
      }
    } else {
      ALOGW(
    ...
    ALOGW("nfa_hci_api_send_rsp pipe:%d not found", p_evt_data->send_rsp.pipe);
  }
  ...
  if ((p_pipe = nfa_hciu_find_pipe_by_pid(p_evt_data->send_evt.pipe)) != NULL) {
    ...
    if ((nfa_hciu_is_active_host(p_pipe->dest_host)) &&
      ...
      if (p_pipe->pipe_state == NFA_HCI_PIPE_OPENED) {
        ...
        ALOGW("nfa_hci_api_send_event pipe:%d not open", p_pipe->pipe_id);
      }
    } else {
      ALOGW(
    ...
    ALOGW("nfa_hci_api_send_event pipe:%d not found",
...
void nfa_hci_handle_admin_gate_rsp(uint8_t* p_data, uint8_t data_len) {
  ...
  if ((nfa_hci_cb.hci_state == NFA_HCI_STATE_STARTUP) ||
    ...
    if (nfa_hci_cb.inst != NFA_HCI_ANY_OK) {
      ALOGE("nfa_hci_handle_admin_gate_rsp - Initialization failed");
...
void nfa_hci_handle_admin_gate_evt(uint8_t* p_data) {
  ...
  if (nfa_hci_cb.inst != NFA_HCI_EVT_HOT_PLUG) {
    ALOGE("nfa_hci_handle_admin_gate_evt - Unknown event on ADMIN Pipe");
    return;
  }

  ALOGD("nfa_hci_handle_admin_gate_evt - HOT PLUG EVT event on ADMIN Pipe");
...
void nfa_hci_handle_dyn_pipe_pkt(uint8_t pipe_id, uint8_t* p_data,
  ...
  if (p_pipe == NULL) {
    /* Invalid pipe ID */
    ALOGE("nfa_hci_handle_dyn_pipe_pkt - Unknown pipe %d", pipe_id);
  ...
  if (p_pipe->local_gate == NFA_HCI_IDENTITY_MANAGEMENT_GATE) {
    ...
    p_gate = nfa_hciu_find_gate_by_gid(p_pipe->local_gate);
    if (p_gate == NULL) {
      ALOGE("nfa_hci_handle_dyn_pipe_pkt - Pipe's gate %d is corrupt",
            p_pipe->local_gate);
      if (nfa_hci_cb.type == NFA_HCI_COMMAND_TYPE)
        nfa_hciu_send_msg(pipe_id, NFA_HCI_RESPONSE_TYPE, NFA_HCI_ANY_E_NOK, 0,
                          NULL);
