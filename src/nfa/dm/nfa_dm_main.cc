...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("nfa_dm_evt_hdlr event: %s (0x%02x)", nfa_dm_evt_2_str(event).c_str(),
        event);
#else
  ALOGD("nfa_dm_evt_hdlr event: 0x%x", event);
#endif
...
void nfa_dm_sys_disable(void) {
  ...
  if (nfa_sys_is_graceful_disable()) {
    if ((nfa_dm_cb.disc_cb.disc_state == NFA_DM_RFST_IDLE) &&
      ...
      ALOGW("DM disc_state state = %d disc_flags:0x%x",
...
bool nfa_dm_is_active(void) {
  ALOGD("nfa_dm_is_active () flags:0x%x", nfa_dm_cb.flags);
