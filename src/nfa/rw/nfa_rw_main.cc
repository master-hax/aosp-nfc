...
bool nfa_rw_handle_event(NFC_HDR* p_msg) {
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("nfa_rw_handle_event event: %s (0x%02x), flags: %08x",
        nfa_rw_evt_2_str(p_msg->event).c_str(), p_msg->event, nfa_rw_cb.flags);
#else
  ALOGD("nfa_rw_handle_event event: 0x%x, flags: %08x", p_msg->event,
        nfa_rw_cb.flags);
#endif
  ...
  if (act_idx < (NFA_RW_MAX_EVT & 0xFF)) {
    ...
    ALOGE("nfa_rw_handle_event: unhandled event 0x%02X", p_msg->event);
    return true;
  }
}
