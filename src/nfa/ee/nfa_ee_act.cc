...
static void nfa_ee_trace_aid(std::string p_str, uint8_t id, uint8_t aid_len,
  ...
  if (aid_len > NFA_MAX_AID_LEN) {
    ALOGE("aid_len: %d exceeds max(%d)", aid_len, NFA_MAX_AID_LEN);
    len = NFA_MAX_AID_LEN;
  }
  for (xx = 0; xx < len; xx++) {
  ...
  ALOGD("%s id:0x%x len=%d aid:%s", p_str.c_str(), id, aid_len, buff);
}
...
static void nfa_ee_update_route_size(tNFA_EE_ECB* p_cb) {
  ...
  ALOGD("nfa_ee_update_route_size nfcee_id:0x%x size_mask:%d", p_cb->nfcee_id,
        p_cb->size_mask);
}
...
static void nfa_ee_update_route_aid_size(tNFA_EE_ECB* p_cb) {
  ...
  ALOGD("nfa_ee_update_route_aid_size nfcee_id:0x%x size_aid:%d",
...
static uint16_t nfa_ee_total_lmrt_size(void) {
  ...
  ALOGD("nfa_ee_total_lmrt_size size:%d", lmrt_size);
  return lmrt_size;
}
...
static void nfa_ee_add_proto_route_to_ecb(tNFA_EE_ECB* p_cb, uint8_t* pp,
  ...
  if (p_cb->nfcee_id == NFC_DH_ID) {
    ...
    num_tlv++;
    ALOGD("%s - NFC DEP added for DH!!!", __func__);
  }
  /* update the num_tlv and current offset */
  uint8_t entry_size = (uint8_t)(pp - p);
  *p_cur_offset += entry_size;
  ...
  if (p_cb->aid_entries) {
    ...
    for (int xx = 0; xx < p_cb->aid_entries; xx++) {
      ...
      if (p_cb->aid_rt_info[xx] & NFA_EE_AE_ROUTE) {
        ...
        ALOGD("%s -  p_cb->aid_info%x", __func__, p_cb->aid_info[xx]);
    ...
    ALOGD("%s - No AID entries available", __func__);
  }
}
...
static void nfa_ee_conn_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  ALOGD("nfa_ee_conn_cback: conn_id: %d, event=0x%02x", conn_id, event);
...
void nfa_ee_api_discover(tNFA_EE_MSG* p_data) {
  ...
  ALOGD("nfa_ee_api_discover() in_use:%d", nfa_ee_cb.discv_timer.in_use);
...
void nfa_ee_api_mode_set(tNFA_EE_MSG* p_data) {
  ...
  ALOGD("nfa_ee_api_mode_set() handle:0x%02x mode:%d", p_cb->nfcee_id,
...
void nfa_ee_api_set_tech_cfg(tNFA_EE_MSG* p_data) {
  ...
  if (nfa_ee_total_lmrt_size() > NFC_GetLmrtSize()) {
    ALOGE("nfa_ee_api_set_tech_cfg Exceed LMRT size");
...
void nfa_ee_api_set_proto_cfg(tNFA_EE_MSG* p_data) {
  ...
  nfa_ee_update_route_size(p_cb);
  if (nfa_ee_total_lmrt_size() > NFC_GetLmrtSize()) {
    ALOGE("nfa_ee_api_set_proto_cfg Exceed LMRT size");
    evt_data.status = NFA_STATUS_BUFFER_FULL;
...
void nfa_ee_api_add_aid(tNFA_EE_MSG* p_data) {
  ...
  if (p_chk_cb) {
    ALOGD("nfa_ee_api_add_aid The AID entry is already in the database");
    if (p_chk_cb == p_cb) {
      ...
      if (new_size > NFC_GetLmrtSize()) {
        ALOGE("Exceed LMRT size:%d (add ROUTE)", new_size);
        evt_data.status = NFA_STATUS_BUFFER_FULL;
        p_cb->aid_rt_info[entry] &= ~NFA_EE_AE_ROUTE;
      } else {
    ...
    if ((len_needed + len) > NFA_EE_MAX_AID_CFG_LEN) {
      ...
      if (new_size > NFC_GetLmrtSize()) {
        ALOGE("Exceed LMRT size:%d", new_size);
        evt_data.status = NFA_STATUS_BUFFER_FULL;
      } else {
        /* add AID */
      ...
      ALOGE("Exceed NFA_EE_MAX_AID_ENTRIES:%d", NFA_EE_MAX_AID_ENTRIES);
      evt_data.status = NFA_STATUS_BUFFER_FULL;
    }
  }
  ...
  ALOGD("status:%d ee_cfged:0x%02x ", evt_data.status, nfa_ee_cb.ee_cfged);
...
void nfa_ee_api_remove_aid(tNFA_EE_MSG* p_data) {
  ...
  if (p_cb && p_cb->aid_entries) {
    ALOGD("aid_rt_info[%d]: 0x%02x", entry, p_cb->aid_rt_info[entry]);
    ...
    if ((entry + 1) < p_cb->aid_entries) {
      ...
      ALOGD("nfa_ee_api_remove_aid len:%d, rest_len:%d", len, rest_len);
    ...
    ALOGE("nfa_ee_api_remove_aid The AID entry is not in the database");
...
void nfa_ee_api_lmrt_size(tNFA_EE_MSG* p_data) {
  ...
  ALOGD("nfa_ee_api_lmrt_size total size:%d remaining size:%d", total_size,
        evt_data.size);
...
static void nfa_ee_remove_pending(void) {
  ...
  ALOGD("nfa_ee_remove_pending() cur_ee:%d, num_removed:%d first_removed:%d",
...
void nfa_ee_nci_disc_rsp(tNFA_EE_MSG* p_data) {
  ...
  ALOGD("nfa_ee_nci_disc_rsp() em_state:%d cur_ee:%d, num_nfcee:%d",
  ...
  if (p_evt->status == NFC_STATUS_OK) {
    ...
    if (nfa_ee_cb.num_ee_expecting > NFA_EE_MAX_EE_SUPPORTED) {
      ALOGE("NFA-EE num_ee_expecting:%d > max:%d", nfa_ee_cb.num_ee_expecting,
            NFA_EE_MAX_EE_SUPPORTED);
    }
  }
  nfa_ee_report_disc_done(notify_enable_done);
  ALOGD("nfa_ee_nci_disc_rsp() em_state:%d cur_ee:%d num_ee_expecting:%d",
...
void nfa_ee_nci_disc_ntf(tNFA_EE_MSG* p_data) {
  ...
  switch (nfa_ee_cb.em_state) {
    ...
    case NFA_EE_EM_STATE_INIT_DONE:
      ...
      if (p_cb == NULL) {
        ...
        ALOGD("cur_ee:%d ecb_flags=0x%02x  ee_status=0x%x", nfa_ee_cb.cur_ee,
              p_cb->ecb_flags, p_cb->ee_status);
      }
      break;
  ...
  ALOGD("nfa_ee_nci_disc_ntf cur_ee:%d", nfa_ee_cb.cur_ee);
...
static void nfa_ee_build_discover_req_evt(tNFA_EE_DISCOVER_REQ* p_evt_data) {
  ...
  for (xx = 0; xx < nfa_ee_cb.cur_ee; xx++, p_cb++) {
    ...
    ALOGD("[%d] ee_handle:0x%x, listen protocol A:%d, B:%d, F:%d, BP:%d",
...
static void nfa_ee_report_discover_req_evt(void) {
  ...
  if (!nfa_dm_is_active()) {
    ALOGD("nfa_ee_report_discover_req_evt DM is not active");
    return;
  }
...
void nfa_ee_nci_mode_set_rsp(tNFA_EE_MSG* p_data) {
  ...
  ALOGD("%s handle:0x%02x mode:%d", __func__, p_rsp->nfcee_id, p_rsp->mode);
  p_cb = nfa_ee_find_ecb(p_rsp->nfcee_id);
  if (p_cb == NULL) {
    ALOGE("%s Can not find cb for handle:0x%02x", __func__, p_rsp->nfcee_id);
    return;
  }

  /* update routing table and vs on mode change */
  nfa_ee_start_timer();
  ALOGE("%s p_rsp->status:0x%02x", __func__, p_rsp->status);
  if (p_rsp->status == NFA_STATUS_OK) {
    if (p_rsp->mode == NFA_EE_MD_ACTIVATE) {
      ...
      if (p_cb->tech_switch_on | p_cb->tech_switch_off |
        ...
        ALOGD("deactivating/still configured. Force update");
      }
  ...
  ALOGD("status:%d ecb_flags  :0x%02x ee_cfged:0x%02x ee_status:%d",
...
void nfa_ee_report_update_evt(void) {
  ...
  ALOGD("nfa_ee_report_update_evt ee_wait_evt:0x%x wait_rsp:%d",
        nfa_ee_cb.ee_wait_evt, nfa_ee_cb.wait_rsp);
...
void nfa_ee_nci_wait_rsp(tNFA_EE_MSG* p_data) {
  ...
  ALOGD("nfa_ee_nci_wait_rsp() ee_wait_evt:0x%x wait_rsp:%d",
        nfa_ee_cb.ee_wait_evt, nfa_ee_cb.wait_rsp);
...
void nfa_ee_nci_disc_req_ntf(tNFA_EE_MSG* p_data) {
  ...
  ALOGD("nfa_ee_nci_disc_req_ntf () num_info: %d cur_ee:%d", p_cbk->num_info,
        nfa_ee_cb.cur_ee);
  ...
  for (xx = 0; xx < p_cbk->num_info; xx++) {
    ...
    if (!p_cb) {
      ALOGD("Cannot find cb for NFCEE: 0x%x", p_cbk->info[xx].nfcee_id);
      p_cb = nfa_ee_find_ecb(NFA_EE_INVALID);
      if (p_cb) {
        p_cb->nfcee_id = p_cbk->info[xx].nfcee_id;
        p_cb->ecb_flags |= NFA_EE_ECB_FLAGS_ORDER;
      } else {
        ALOGE("Cannot allocate cb for NFCEE: 0x%x", p_cbk->info[xx].nfcee_id);
        continue;
      }
    } else {
...
void nfa_ee_get_tech_route(uint8_t power_state, uint8_t* p_handles) {
  ...
  ALOGD("nfa_ee_get_tech_route(): %d", power_state);
  ...
  ALOGD("0x%x, 0x%x, 0x%x, 0x%x", p_handles[0], p_handles[1], p_handles[2],
        p_handles[3]);
}
...
tNFA_STATUS nfa_ee_route_add_one_ecb(tNFA_EE_ECB* p_cb, int* p_max_len,
  ...
  *ps = num_tlv;
  ...
  ALOGD("ee_cfg_sts:0x%02x lmrt_size:%d", nfa_ee_cb.ee_cfg_sts, tlv_size);
  ...
  if (more == false) {
    ...
    if (nfa_ee_cb.ee_cfg_sts & NFA_EE_STS_CHANGED_ROUTING) {
      ...
      ALOGD("nfa_ee_route_add_one_ecb: set routing num_tlv:%d tlv_size:%d",
            num_tlv, tlv_size);
...
void nfa_ee_route_add_one_ecb_by_route_order(tNFA_EE_ECB* p_cb, int rout_type,
  ...
  /* use the first byte of the buffer (ps) to keep the num_tlv */
  uint8_t num_tlv = *ps;
  ALOGD("%s - max_len:%d, cur_offset:%d, more:%d, num_tlv:%d,rout_type:- %d",
  ...
  switch (rout_type) {
    ...
    default: { ALOGD("%s -  Route type - NA:- %d", __func__, rout_type); }
  }
  ...
  if (more == false) {
    ...
    if (nfa_ee_cb.ee_cfg_sts & NFA_EE_STS_CHANGED_ROUTING) {
      ...
      ALOGD("%s : set routing num_tlv:%d tlv_size:%d", __func__, num_tlv,
            tlv_size);
      if (NFC_SetRouting(more, num_tlv, (uint8_t)(*p_cur_offset), ps + 1) ==
          NFA_STATUS_OK) {
        nfa_ee_cb.wait_rsp++;
...
static bool nfa_ee_need_recfg(void) {
  ...
  ALOGD("nfa_ee_need_recfg() ee_cfged: 0x%02x ee_cfg_sts: 0x%02x",
  ...
  if (nfa_ee_cb.ee_cfged || nfa_ee_cb.ee_cfg_sts) {
    if (nfa_ee_cb.ee_cfg_sts & NFA_EE_STS_CHANGED) {
      ...
      for (xx = 0; xx <= nfa_ee_cb.cur_ee; xx++) {
        ALOGD("%d: ecb_flags  : 0x%02x, mask: 0x%02x", xx, p_cb->ecb_flags,
              mask);
...
void nfa_ee_lmrt_to_nfcc(tNFA_EE_MSG* p_data) {
  ...
  if (p == NULL) {
    ALOGE("nfa_ee_lmrt_to_nfcc() no buffer to send routing info.");
  ...
  for (xx = 0; xx < nfa_ee_cb.cur_ee; xx++, p_cb--) {
    if (p_cb->ee_status == NFC_NFCEE_STATUS_ACTIVE) {
      if (last_active == NFA_EE_INVALID) {
        ...
        ALOGD("last_active: 0x%x", last_active);
      }
    }
  }
  if (last_active == NFA_EE_INVALID) {
  ...
  *p = 0;
  for (int rt = NCI_ROUTE_ORDER_AID; rt <= NCI_ROUTE_ORDER_TECHNOLOGY; rt++) {
    ...
    for (xx = 0; (xx < nfa_ee_cb.cur_ee) && check; xx++, p_cb++) {
      if (p_cb->ee_status == NFC_NFCEE_STATUS_ACTIVE) {
        ALOGD("%s --add the routing for NFCEEs!!", __func__);
    ...
    ALOGD("%s --add the routing for DH!!", __func__);
...
void nfa_ee_update_rout(void) {
  ...
  ALOGD("nfa_ee_update_rout ee_cfg_sts:0x%02x", nfa_ee_cb.ee_cfg_sts);
  ...
  ALOGD("nfa_ee_update_rout ee_cfg_sts:0x%02x ee_cfged:0x%02x",
