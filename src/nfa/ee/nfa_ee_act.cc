...
static void nfa_ee_update_route_sys_code_size(tNFA_EE_ECB* p_cb) {
  ...
  if (p_cb->sys_code_cfg_entries) {
    for (xx = 0; xx < p_cb->sys_code_cfg_entries; xx++) {
...
int nfa_ee_find_total_sys_code_len(tNFA_EE_ECB* p_cb, int start_entry) {
  ...
  if (p_cb->sys_code_cfg_entries > start_entry) {
    for (xx = start_entry; xx < p_cb->sys_code_cfg_entries; xx++) {
...
tNFA_EE_ECB* nfa_ee_find_sys_code_offset(uint8_t* p_sys_code, int* p_offset,
                                         int* p_entry) {
  uint8_t xx, yy, offset, mask;
  tNFA_EE_ECB *p_ret = NULL, *p_ecb;

  for (xx = 0; xx < NFA_EE_NUM_ECBS; xx++) {
    mask = 0;
    p_ecb = &nfa_ee_cb.ecb[xx];
    mask = nfa_ee_ecb_to_mask(p_ecb);
    if ((nfa_ee_cb.ee_cfged & mask) == 0)
      continue; /*try next ecb*/
    else {
      if (p_ecb->sys_code_cfg_entries) {
        offset = 0;
        for (yy = 0; yy < p_ecb->sys_code_cfg_entries; yy++) {
...
void nfa_ee_api_add_sys_code(tNFA_EE_MSG* p_data) {
  int offset = 0, entry = 0, len = 0;
  tNFA_EE_ECB* p_cb = p_data->cfg_hdr.p_cb;
  tNFA_EE_ECB* p_chk_cb = NULL;
  tNFA_EE_CBACK_DATA evt_data = {0};
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s id:0x%x SC:0x%X 0x%0X", __func__, p_add->nfcee_id,
                      p_add->p_syscode[0], p_add->p_syscode[1]);

  p_chk_cb = nfa_ee_find_sys_code_offset(p_add->p_syscode, &offset, &entry);
  ...
  if (p_chk_cb) {
    ...
    len = nfa_all_ee_find_total_sys_code_len();
    /* make sure the control block has enough room to hold this entry */
    if ((NFA_EE_SYSTEM_CODE_LEN + len) > NFA_EE_MAX_SYSTEM_CODE_CFG_LEN) {
      ...
      new_size = nfa_ee_total_lmrt_size() + NFA_EE_SYSTEM_CODE_TLV_SIZE;
      if (new_size > NFC_GetLmrtSize()) {
  ...
  LOG(ERROR) << StringPrintf("%s: status:%d ee_cfged:0x%02x ", __func__,
...
void nfa_ee_api_remove_sys_code(tNFA_EE_MSG* p_data) {
  ...
  p_cb = nfa_ee_find_sys_code_offset(p_data->rm_syscode.p_syscode, &offset,
                                     &entry);
  ...
  if (p_cb && p_cb->sys_code_cfg_entries) {
    ...
    if ((entry + 1) < p_cb->sys_code_cfg_entries) {
      ...
      /* Find the total len from the next entry to the last one */
      total_len = nfa_ee_find_total_sys_code_len(p_cb, entry + 1);

      rm_len = NFA_EE_SYSTEM_CODE_LEN;
