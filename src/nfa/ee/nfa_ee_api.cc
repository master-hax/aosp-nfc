...
tNFA_STATUS NFA_EeDiscover(tNFA_EE_CBACK* p_cback) {
  ...
  ALOGD("NFA_EeDiscover()");

  if (nfa_ee_cb.em_state != NFA_EE_EM_STATE_INIT_DONE) {
    ALOGE("NFA_EeDiscover bad em state: %d", nfa_ee_cb.em_state);
    status = NFA_STATUS_FAILED;
  } else if ((nfa_ee_cb.p_ee_disc_cback != NULL) || (p_cback == NULL)) {
    ALOGE("NFA_EeDiscover() in progress or NULL callback function");
...
tNFA_STATUS NFA_EeGetInfo(uint8_t* p_num_nfcee, tNFA_EE_INFO* p_info) {
  ...
  ALOGD("NFA_EeGetInfo em_state:%d cur_ee:%d", nfa_ee_cb.em_state,
        nfa_ee_cb.cur_ee);
  /* validate parameters */
  if (p_info == NULL || p_num_nfcee == NULL) {
    ALOGE("NFA_EeGetInfo bad parameter");
    return (NFA_STATUS_INVALID_PARAM);
  }
  max_ret = *p_num_nfcee;
  *p_num_nfcee = 0;
  if (nfa_ee_cb.em_state == NFA_EE_EM_STATE_INIT) {
    ALOGE("NFA_EeGetInfo bad em state: %d", nfa_ee_cb.em_state);
    return (NFA_STATUS_FAILED);
  }

  /* compose output */
  for (xx = 0; (xx < ret) && (num_ret < max_ret); xx++, p_cb++) {
    ALOGD("xx:%d max_ret:%d, num_ret:%d ee_status:0x%x", xx, max_ret, num_ret,
          p_cb->ee_status);
...
tNFA_STATUS NFA_EeRegister(tNFA_EE_CBACK* p_cback) {
  ...
  ALOGD("NFA_EeRegister()");

  if (p_cback == NULL) {
    ALOGE("NFA_EeRegister(): with NULL callback function");
    status = NFA_STATUS_INVALID_PARAM;
  } else {
...
tNFA_STATUS NFA_EeDeregister(tNFA_EE_CBACK* p_cback) {
  ...
  ALOGD("NFA_EeDeregister() %d, status:%d", index, status);
...
tNFA_STATUS NFA_EeModeSet(tNFA_HANDLE ee_handle, tNFA_EE_MD mode) {
  ...
  ALOGD("NFA_EeModeSet(): handle:<0x%x>, mode:0x%02X", ee_handle, mode);

  if (p_found == NULL) {
    ALOGE("NFA_EeModeSet() invalid NFCEE:0x%04x", ee_handle);
    status = NFA_STATUS_INVALID_PARAM;
  } else {
...
tNFA_STATUS NFA_EeAddAidRouting(tNFA_HANDLE ee_handle, uint8_t aid_len,
  ...
  ALOGD("NFA_EeAddAidRouting(): handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);
  ...
  if ((p_cb == NULL) || (aid_len == 0) || (p_aid == NULL) ||
    ...
    ALOGE("Bad ee_handle or AID (len=%d)", aid_len);
    status = NFA_STATUS_INVALID_PARAM;
  } else {
    p_msg = (tNFA_EE_API_ADD_AID*)GKI_getbuf(size);
    if (p_msg != NULL) {
      ALOGD("aid:<%02x%02x>", p_aid[0], p_aid[1]);
...
tNFA_STATUS NFA_EeGetLmrtRemainingSize(void) {
  ...
  ALOGD("NFA_EeGetLmrtRemainingSize()");
...
tNFA_STATUS NFA_EeConnect(tNFA_HANDLE ee_handle, uint8_t ee_interface,
  ...
  ALOGD("NFA_EeConnect(): handle:<0x%x> ee_interface:0x%x", ee_handle,
        ee_interface);
  p_cb = nfa_ee_find_ecb(nfcee_id);

  if ((p_cb == NULL) || (p_cback == NULL)) {
    ALOGE("Bad ee_handle or NULL callback function");
    status = NFA_STATUS_INVALID_PARAM;
  } else {
...
tNFA_STATUS NFA_EeSendData(tNFA_HANDLE ee_handle, uint16_t data_len,
  ...
  ALOGD("NFA_EeSendData(): handle:<0x%x>", ee_handle);

  p_cb = nfa_ee_find_ecb(nfcee_id);

  if ((p_cb == NULL) || (p_cb->conn_st != NFA_EE_CONN_ST_CONN) ||
      (p_data == NULL)) {
    ALOGE("Bad ee_handle or NULL data");
    status = NFA_STATUS_INVALID_PARAM;
  } else {
...
tNFA_STATUS NFA_EeDisconnect(tNFA_HANDLE ee_handle) {
  ...
  ALOGD("NFA_EeDisconnect(): handle:<0x%x>", ee_handle);
  p_cb = nfa_ee_find_ecb(nfcee_id);

  if ((p_cb == NULL) || (p_cb->conn_st != NFA_EE_CONN_ST_CONN)) {
    ALOGE("NFA_EeDisconnect() Bad ee_handle");
    status = NFA_STATUS_INVALID_PARAM;
  } else {
