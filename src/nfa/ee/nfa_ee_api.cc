...
tNFA_STATUS NFA_EeSetDefaultTechRouting(
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      ""
      "handle:<0x%x>technology_mask:<0x%02x>/<0x%02x>/<0x%02x>",
...
tNFA_STATUS NFA_EeSetDefaultProtoRouting(
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "handle:<0x%x>protocol_mask:<0x%02x>/<0x%02x>/<0x%02x>", ee_handle,
      protocols_switch_on, protocols_switch_off, protocols_battery_off);
  p_cb = nfa_ee_find_ecb(nfcee_id);
