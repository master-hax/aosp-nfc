...
void nfa_ee_route_add_one_ecb_by_route_order(tNFA_EE_ECB* p_cb, int rout_type,
  ...
  num_tlv = *ps;
  NFA_TRACE_DEBUG6(
      "nfa_ee_route_add_one_ecb_by_route_order max_len:%d, max_tlv:%d, "
      "cur_offset:%d, "
    ...
    case NCI_ROUTE_ORDER_PROTOCOL: {
      ...
      if (p_cb->nfcee_id == NFC_DH_ID) {
        ...
        *pp++ = NFC_PROTOCOL_NFC_DEP;
        num_tlv++;
        NFA_TRACE_DEBUG0(
            "nfa_ee_route_add_one_ecb_by_route_order --- NFC DEP added for "
            "DH!!!");
      }
      /* update the num_tlv and current offset */
      entry_size = (uint8_t)(pp - p);
      ...
      if (p_cb->aid_entries) {
        ...
        for (xx = 0; xx < p_cb->aid_entries; xx++) {
          ...
          if (p_cb->aid_rt_info[xx] & NFA_EE_AE_ROUTE) {
            ...
            len = *pa++; /* aid_len */
            NFA_TRACE_DEBUG1(
                "nfa_ee_route_add_one_ecb_by_route_order p_cb->aid_info%x",
                p_cb->aid_info[xx]);
            if (p_cb->aid_info[xx] & NCI_ROUTE_QUAL_LONG_SELECT) {
              NFA_TRACE_DEBUG1(
                  "nfa_ee_route_add_one_ecb_by_route_ordersuraj %x",
            ...
            if (p_cb->aid_info[xx] & NCI_ROUTE_QUAL_SHORT_SELECT) {
              NFA_TRACE_DEBUG1(
                  "nfa_ee_route_add_one_ecb_by_route_ordersuraj2 %x",
        ...
        NFA_TRACE_DEBUG0(
            "nfa_ee_route_add_one_ecb_by_route_order --- No AID Enteries "
            "Available");
      }
    } break;
    default: {
      NFA_TRACE_DEBUG1(
          "nfa_ee_route_add_one_ecb_by_route_order -- Rout type- NA:- %d",
          rout_type);
    }
  }
  ...
  if (more == false) {
    ...
    if (nfa_ee_cb.ee_cfg_sts & NFA_EE_STS_CHANGED_ROUTING) {
      ...
      NFA_TRACE_DEBUG2(
          "nfa_ee_route_add_one_ecb_by_route_order: set routing num_tlv:%d "
          "tlv_size:%d",
          num_tlv, tlv_size);
      if (NFC_SetRouting(more, num_tlv, (uint8_t)(*p_cur_offset), ps + 1) ==
          NFA_STATUS_OK) {
