...
tNFA_STATUS NFA_P2pRegisterServer(uint8_t server_sap,
  ...
  ALOGD("NFA_P2pRegisterServer (): server_sap:0x%02x, link_type:0x%x, SN:<%s>",
  ...
  if ((server_sap != NFA_P2P_ANY_SAP) &&
    ...
    ALOGE("NFA_P2pRegisterServer (): server_sap must be between %d and %d",
          LLCP_SAP_SDP + 1, LLCP_UPPER_BOUND_SDP_SAP);
    return (NFA_STATUS_FAILED);
  } else if (((link_type & NFA_P2P_LLINK_TYPE) == 0x00) &&
             ((link_type & NFA_P2P_DLINK_TYPE) == 0x00)) {
    ALOGE("NFA_P2pRegisterServer(): link type (0x%x) must be specified",
          link_type);
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_P2pRegisterClient(tNFA_P2P_LINK_TYPE link_type,
  ...
  ALOGD("NFA_P2pRegisterClient (): link_type:0x%x", link_type);

  if (((link_type & NFA_P2P_LLINK_TYPE) == 0x00) &&
      ((link_type & NFA_P2P_DLINK_TYPE) == 0x00)) {
    ALOGE("NFA_P2pRegisterClient (): link type (0x%x) must be specified",
          link_type);
    return (NFA_STATUS_FAILED);
  }
...
tNFA_STATUS NFA_P2pDeregister(tNFA_HANDLE handle) {
  ...
  ALOGD("NFA_P2pDeregister (): handle:0x%02X", handle);

  xx = handle & NFA_HANDLE_MASK;

  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    ALOGE("NFA_P2pDeregister (): Handle is invalid or not registered");
...
tNFA_STATUS NFA_P2pAcceptConn(tNFA_HANDLE handle, uint16_t miu, uint8_t rw) {
  ...
  ALOGD("NFA_P2pAcceptConn (): handle:0x%02X, MIU:%d, RW:%d", handle, miu, rw);

  xx = handle & NFA_HANDLE_MASK;

  if (!(xx & NFA_P2P_HANDLE_FLAG_CONN)) {
    ALOGE("NFA_P2pAcceptConn (): Connection Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  } else {
  ...
  if ((miu < LLCP_DEFAULT_MIU) || (nfa_p2p_cb.local_link_miu < miu)) {
    ALOGE("NFA_P2pAcceptConn (): MIU(%d) must be between %d and %d", miu,
...
tNFA_STATUS NFA_P2pRejectConn(tNFA_HANDLE handle) {
  ...
  ALOGD("NFA_P2pRejectConn (): handle:0x%02X", handle);

  xx = handle & NFA_HANDLE_MASK;

  if (!(xx & NFA_P2P_HANDLE_FLAG_CONN)) {
    ALOGE("NFA_P2pRejectConn (): Connection Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  } else {
    xx &= ~NFA_P2P_HANDLE_FLAG_CONN;
  }
...
tNFA_STATUS NFA_P2pDisconnect(tNFA_HANDLE handle, bool flush) {
  ...
  ALOGD("NFA_P2pDisconnect (): handle:0x%02X, flush=%d", handle, flush);
  ...
  if (xx & NFA_P2P_HANDLE_FLAG_CONN) {
    ...
    if ((xx >= LLCP_MAX_DATA_LINK) || (nfa_p2p_cb.conn_cb[xx].flags == 0)) {
      ALOGE("NFA_P2pDisconnect (): Connection Handle is not valid");
      return (NFA_STATUS_BAD_HANDLE);
    }
  } else {
    ALOGE("NFA_P2pDisconnect (): Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  }
...
tNFA_STATUS NFA_P2pConnectByName(tNFA_HANDLE client_handle,
  ...
  ALOGD("NFA_P2pConnectByName (): client_handle:0x%x, SN:<%s>, MIU:%d, RW:%d",
        client_handle, p_service_name, miu, rw);

  xx = client_handle & NFA_HANDLE_MASK;

  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    ALOGE("NFA_P2pConnectByName (): Client Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  }
...
tNFA_STATUS NFA_P2pConnectBySap(tNFA_HANDLE client_handle, uint8_t dsap,
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    ALOGE("NFA_P2pConnectBySap (): Client Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  }
...
tNFA_STATUS NFA_P2pSendUI(tNFA_HANDLE handle, uint8_t dsap, uint16_t length,
  ...
  ALOGD("NFA_P2pSendUI (): handle:0x%X, DSAP:0x%02X, length:%d", handle, dsap,
        length);
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    ALOGE("NFA_P2pSendUI (): Handle (0x%X) is not valid", handle);
    ...
    ALOGW("NFA_P2pSendUI(): handle:0x%X, logical data link is congested",
          handle);
    ret_status = NFA_STATUS_CONGESTED;
...
tNFA_STATUS NFA_P2pReadUI(tNFA_HANDLE handle, uint32_t max_data_len,
  ...
  ALOGD("NFA_P2pReadUI (): handle:0x%X", handle);
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    ALOGE("NFA_P2pReadUI (): Handle (0x%X) is not valid", handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
  } else {
...
tNFA_STATUS NFA_P2pFlushUI(tNFA_HANDLE handle, uint32_t* p_length) {
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    ALOGE("NFA_P2pFlushUI (): Handle (0x%X) is not valid", handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
    *p_length = 0;
  } else {
...
tNFA_STATUS NFA_P2pSendData(tNFA_HANDLE handle, uint16_t length,
  ...
  ALOGD("NFA_P2pSendData (): handle:0x%X, length:%d", handle, length);
  ...
  if ((!(handle & NFA_P2P_HANDLE_FLAG_CONN)) || (xx >= LLCP_MAX_DATA_LINK) ||
    ...
    ALOGE("NFA_P2pSendData (): Handle(0x%X) is not valid", handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
  } else if (nfa_p2p_cb.conn_cb[xx].flags & NFA_P2P_CONN_FLAG_REMOTE_RW_ZERO) {
    ALOGE("NFA_P2pSendData (): handle:0x%X, Remote set RW to 0 (flow off)",
          handle);
    ret_status = NFA_STATUS_FAILED;
  } else if (nfa_p2p_cb.conn_cb[xx].remote_miu < length) {
    ALOGE("NFA_P2pSendData (): handle:0x%X, Data more than remote MIU(%d)",
    ...
    ALOGW("NFA_P2pSendData (): handle:0x%X, data link connection is congested",
          handle);
    ret_status = NFA_STATUS_CONGESTED;
...
tNFA_STATUS NFA_P2pReadData(tNFA_HANDLE handle, uint32_t max_data_len,
  ...
  ALOGD("NFA_P2pReadData (): handle:0x%X", handle);
  ...
  if ((!(handle & NFA_P2P_HANDLE_FLAG_CONN)) || (xx >= LLCP_MAX_DATA_LINK) ||
      (nfa_p2p_cb.conn_cb[xx].flags == 0)) {
    ALOGE("NFA_P2pReadData (): Handle(0x%X) is not valid", handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
  } else {
...
tNFA_STATUS NFA_P2pFlushData(tNFA_HANDLE handle, uint32_t* p_length) {
  ...
  ALOGD("NFA_P2pFlushData (): handle:0x%X", handle);
  ...
  if ((!(handle & NFA_P2P_HANDLE_FLAG_CONN)) || (xx >= LLCP_MAX_DATA_LINK) ||
      (nfa_p2p_cb.conn_cb[xx].flags == 0)) {
    ALOGE("NFA_P2pFlushData (): Handle(0x%X) is not valid", handle);
    ret_status = NFA_STATUS_BAD_HANDLE;
  } else {
...
tNFA_STATUS NFA_P2pSetLocalBusy(tNFA_HANDLE conn_handle, bool is_busy) {
  ...
  ALOGD("NFA_P2pSetLocalBusy (): conn_handle:0x%02X, is_busy:%d", conn_handle,
        is_busy);

  xx = conn_handle & NFA_HANDLE_MASK;

  if (!(xx & NFA_P2P_HANDLE_FLAG_CONN)) {
    ALOGE("NFA_P2pSetLocalBusy (): Connection Handle is not valid");
    return (NFA_STATUS_BAD_HANDLE);
  } else {
    xx &= ~NFA_P2P_HANDLE_FLAG_CONN;
  }
...
tNFA_STATUS NFA_P2pGetLinkInfo(tNFA_HANDLE handle) {
  ...
  ALOGD("NFA_P2pGetLinkInfo (): handle:0x%x", handle);

  if (nfa_p2p_cb.llcp_state != NFA_P2P_LLCP_STATE_ACTIVATED) {
    ALOGE("NFA_P2pGetLinkInfo (): LLCP link is not activated");
    return (NFA_STATUS_FAILED);
  }
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    ALOGE("NFA_P2pGetLinkInfo (): Handle is invalid or not registered");
    return (NFA_STATUS_BAD_HANDLE);
  }
...
tNFA_STATUS NFA_P2pGetRemoteSap(tNFA_HANDLE handle, char* p_service_name) {
  ...
  ALOGD("NFA_P2pGetRemoteSap(): handle:0x%x, SN:<%s>", handle, p_service_name);

  if (nfa_p2p_cb.llcp_state != NFA_P2P_LLCP_STATE_ACTIVATED) {
    ALOGE("NFA_P2pGetRemoteSap(): LLCP link is not activated");
    return (NFA_STATUS_FAILED);
  }
  ...
  if ((xx >= NFA_P2P_NUM_SAP) || (nfa_p2p_cb.sap_cb[xx].p_cback == NULL)) {
    ALOGE("NFA_P2pGetRemoteSap (): Handle is invalid or not registered");
    return (NFA_STATUS_BAD_HANDLE);
  }
...
tNFA_STATUS NFA_P2pSetLLCPConfig(uint16_t link_miu, uint8_t opt, uint8_t wt,
  ...
  if (nfa_p2p_cb.llcp_state == NFA_P2P_LLCP_STATE_ACTIVATED) {
    ALOGE("NFA_P2pSetLLCPConfig (): LLCP link is activated");
    return (NFA_STATUS_FAILED);
  }
