...
/* main state */
#define RW_MFC_STATE_NOT_ACTIVATED \
  0x00 /* Mifare Classic is not activated                 */
#define RW_MFC_STATE_IDLE 0x01 /* waiting for upper layer API          */
#define RW_MFC_STATE_DETECT_NDEF                                             \
  0x02                                /* performing NDEF detection precedure \
                                       */
#define RW_MFC_STATE_READ_NDEF 0x03   /* performing read NDEF procedure       */
#define RW_MFC_STATE_UPDATE_NDEF 0x04 /* performing update NDEF procedure */
#define RW_MFC_STATE_PRESENCE_CHECK \
  0x05 /* checking presence of tag             */
#define RW_MFC_STATE_SET_READ_ONLY \
  0x06 /* convert tag to read only             */
 ...
 *tag,
 **      and retrieve the tag's NDEF attribute information.
 **
...
tNFC_STATUS rw_mfc_select(uint8_t selres, uint8_t uid[MFC_UID_LEN]) {
  ...
  if (p_mfc->p_cur_cmd_buf == NULL) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("rw_mfc_select (1)");
    if ((p_mfc->p_cur_cmd_buf = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID)) ==
        NULL) {
      LOG(ERROR) << StringPrintf(
          "rw_mfc_select: rw_mfc_select: unable to allocate buffer for "
          "retransmission");
...
static bool rw_mfc_send_to_lower(NFC_HDR* p_data) {
  ...
  if (NFC_SendData(NFC_RF_CONN_ID, p_data) != NFC_STATUS_OK) {
    LOG(ERROR) << StringPrintf(
        "rw_mfc_send_to_lower (): NFC_SendData () failed");
    return false;
  }
...
void rw_mfc_process_timeout(TIMER_LIST_ENT* p_tle) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_mfc_process_timeout () event=%d", p_tle->event);

  if (p_tle->event == NFC_TTYPE_RW_MFC_RESPONSE) {
    rw_mfc_process_error();
  } else {
    LOG(ERROR) << StringPrintf("rw_mfc_process_timeout () unknown event=%d",
                               p_tle->event);
  }
}
...
static void rw_mfc_conn_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "rw_mfc_conn_cback: conn_id=%i, evt=0x%x", conn_id, event);
  /* Only handle static conn_id */
  if (conn_id != NFC_RF_CONN_ID) {
    LOG(ERROR) << StringPrintf(
        "rw_mfc_conn_cback - Not static connection id: =%i", conn_id);
    return;
  }
  ...
  switch (p_mfc->state) {
    ...
    case RW_MFC_STATE_NOT_ACTIVATED:
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("rw_mfc_data_cback (): RW_MFC_STATE_NOT_ACTIVATED");
      /* p_r_apdu may send upper layer */
      break;
    default:
      LOG(ERROR) << StringPrintf("rw_mfc_data_cback (): invalid state=%d",
                                 p_mfc->state);
      break;
  }
}
 ...
 *******************************************************************************/
static tNFC_STATUS rw_MfcLocateTlv(uint8_t tlv_type) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("rw_MfcLocateTlv ()");
  ...
  if (p_mfc->state != RW_MFC_STATE_IDLE) {
    LOG(ERROR) << StringPrintf(
        "Error: Mifare Classic tag not activated or Busy - State: %u",
        p_mfc->state);
  ...
  if ((tlv_type != TAG_LOCK_CTRL_TLV) && (tlv_type != TAG_MEM_CTRL_TLV) &&
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "RW_MfcLocateTlv - Cannot search TLV: 0x%02x", tlv_type);
    return (NFC_STATUS_FAILED);
  }
  ...
  if ((success = rw_mfc_readBlock(p_mfc->next_block.block)) == NFC_STATUS_OK) {
    p_mfc->state = RW_MFC_STATE_DETECT_TLV;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "rw_MfcLocateTlv RW_MFC_STATE_DETECT_TLV( state) = %d", p_mfc->state);
  } else {
    p_mfc->substate = RW_MFC_SUBSTATE_NONE;
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("rw_MfcLocateTlv ( state) = %d", p_mfc->state);
  }
...
static bool rw_mfc_authenticate(int block, bool KeyA) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_mfc_authenticate (): block : %d", block);
  ...
  if (!mfcbuf) {
    LOG(ERROR) << StringPrintf(
        "rw_mfc_authenticate (): Cannot allocate buffer");
    return false;
  }
...
static tNFC_STATUS rw_mfc_readBlock(int block) {
  ...
  bool status = NFC_STATUS_OK;

  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_mfc_read (): block : %d", block);
  ...
  if (sectorlength != p_mfc->sector_authentified) {
    if ((status = rw_mfc_authenticate(block, true)) == true) {
      LOG(ERROR) << StringPrintf("rw_mfc_read (): RW_MFC_SUBSTATE_WAIT_ACK");
      return NFC_STATUS_OK;
    }
    return NFC_STATUS_FAILED;
  }
  ...
  if (!mfcbuf) {
    LOG(ERROR) << StringPrintf("rw_mfc_read (): Cannot allocate buffer");
  ...
  switch (p_mfc->substate) {
    case RW_MFC_SUBSTATE_WAIT_ACK:
      ...
      if (p[0] == 0x0) {
        ...
        p_mfc->last_block_accessed.auth = false;
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: status=%x", __func__, p[0]);
        nfc_stop_quick_timer(&p_mfc->timer);
        rw_mfc_process_error();
      }
      break;
    ...
    case RW_MFC_STATE_READ_NDEF:
      ...
      if (p_mfc->work_offset == 0) {
        ...
        if (!rw_nfc_decodeTlv(data)) {
          failed = true;
          DLOG_IF(INFO, nfc_debug_enabled)
              << StringPrintf("rw_mfc_handle_read_op FAILED finding TLV");
        }
      }
      ...
      if (p_mfc->work_offset >= p_mfc->ndef_length) {
        ...
        if (rw_mfc_readBlock(p_mfc->next_block.block) != NFC_STATUS_OK) {
          failed = true;
          DLOG_IF(INFO, nfc_debug_enabled)
              << StringPrintf("rw_mfc_handle_read_op FAILED reading next");
        }
      }
 ...
 ** Assuming T & L of TLV will be in the first block messageLength and
 *messageStartIndex
 ** written to the parameters success or failure status is returned.
 **
 ** Returns          true if success
 **
 *******************************************************************************/
static bool rw_nfc_decodeTlv(uint8_t* data) {
  ...
  do {
    if (p[i] == 0x0) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("rw_nfc_decodeTlv: Unknown TLV");
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_nfc_decodeTlv (): i : %d", i);

  if (i < 0 || p[i] != 0x3) {
    LOG(ERROR) << StringPrintf(
        "rw_nfc_decodeTlv: Can't decode message length.");
    return false;
  } else {
    if (p[i + 1] == 0xFF) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "rw_nfc_decodeTlv long  NDEF SIZE= %d", p_mfc->ndef_length);
    } else {
      p_mfc->ndef_length = p[i + 1];
      p_mfc->ndef_start_pos = i + RW_MFC_SHORT_TLV_SIZE;
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "rw_nfc_decodeTlv short  NDEF SIZE= %d", p_mfc->ndef_length);
    }
  }
...
static void rw_mfc_ntf_tlv_detect_complete(tNFC_STATUS status) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_mfc_ntf_tlv_detect_complete ()");
 ...
 *Mifare
 **                  Classic Read commands (if necessary) - depending on the
 *NDEF Msg
 **                  size
 **
...
tNFC_STATUS RW_MfcReadNDef(uint8_t* p_buffer, uint16_t buf_len) {
  ...
  if (p_mfc->ndef_status == MFC_NDEF_NOT_DETECTED) {
    LOG(ERROR) << StringPrintf(
        "RW_MfcReadNDef - Error: NDEF detection not performed yet");
    return (NFC_STATUS_FAILED);
  }

  if (buf_len < p_mfc->ndef_length) {
    LOG(ERROR) << StringPrintf(
        "RW_MfcReadNDef - buffer size: %u  less than NDEF msg sise: %u",
        buf_len, p_mfc->ndef_length);
    return (NFC_STATUS_FAILED);
  }

  if (!p_mfc->ndef_length) {
    LOG(ERROR) << StringPrintf("RW_MfcReadNDef - NDEF Message length is zero ");
  ...
  if ((status = rw_mfc_readBlock(p_mfc->next_block.block)) == NFC_STATUS_OK) {
...
static void rw_mfc_process_error() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("rw_mfc_process_error () State: %u", p_mfc->state);
  ...
  if (rw_cb.cur_retry < RW_MAX_RETRIES) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("Mifare Classic retransmission attempt %i of %i",
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "MFC maximum retransmission attempts reached (%i)", RW_MAX_RETRIES);
  }
  ...
  if (rw_event == RW_MFC_NDEF_DETECT_EVT) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("rw_mfc_process_error status = %x", evt_data.status);
