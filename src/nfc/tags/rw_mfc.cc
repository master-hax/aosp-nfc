...
tNFC_STATUS rw_mfc_select(uint8_t selres, uint8_t uid[MFC_UID_LEN]) {
  ...
  if (p_mfc->p_cur_cmd_buf == NULL) {
    ...
    if (p_mfc->p_cur_cmd_buf == NULL) {
      LOG(ERROR) << __func__ << ": unable to allocate buffer for retransmission";
...
static void rw_mfc_conn_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s conn_id=%i, evt=0x%x", __func__, conn_id, event);
  ...
  switch (p_mfc->state) {
    ...
    case RW_MFC_STATE_NOT_ACTIVATED:
      DLOG_IF(INFO, nfc_debug_enabled) << __func__ << " RW_MFC_STATE_NOT_ACTIVATED";
...
static tNFC_STATUS rw_MfcLocateTlv(uint8_t tlv_type) {
  ...
  if (p_mfc->state != RW_MFC_STATE_IDLE) {
    LOG(ERROR) << __func__ << " Mifare Classic tag not activated or Busy - State:" << p_mfc->state;
  ...
  if ((tlv_type != TAG_LOCK_CTRL_TLV) && (tlv_type != TAG_MEM_CTRL_TLV) &&
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "%s - Cannot search TLV: 0x%02x", __func__, tlv_type);
    return NFC_STATUS_FAILED;
  }
  ...
  if (success == NFC_STATUS_OK) {
    p_mfc->state = RW_MFC_STATE_DETECT_TLV;
    DLOG_IF(INFO, nfc_debug_enabled) << __func__ << " RW_MFC_STATE_DETECT_TLV state="
        << p_mfc->state;
  } else {
    p_mfc->substate = RW_MFC_SUBSTATE_NONE;
    DLOG_IF(INFO, nfc_debug_enabled) << __func__ << " rw_MfcLocateTlv state=" << p_mfc->state;
  }
    ...
    case RW_MFC_STATE_READ_NDEF:
      ...
      if (p_mfc->work_offset >= p_mfc->ndef_length) {
        ...
        if (rw_mfc_readBlock(p_mfc->next_block.block) != NFC_STATUS_OK) {
          failed = true;
          DLOG_IF(INFO, nfc_debug_enabled) << __func__ << " FAILED reading next");
        }
      }
...
static bool rw_nfc_decodeTlv(uint8_t* data) {
  ...
  if (i < 0 || p[i] != 0x3) {
    ...
    if (p[i + 1] == 0xFF) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << __func__ << " long NDEF SIZE="<< p_mfc->ndef_length;
    } else {
      p_mfc->ndef_length = p[i + 1];
      p_mfc->ndef_start_pos = i + RW_MFC_SHORT_TLV_SIZE;
      DLOG_IF(INFO, nfc_debug_enabled) << __func__ << " short NDEF SIZE=" << p_mfc->ndef_length;
    }
  }
  return true;
}
...
tNFC_STATUS RW_MfcReadNDef(uint8_t* p_buffer, uint16_t buf_len) {
  ...
  if (buf_len < p_mfc->ndef_length) {
    LOG(ERROR) << __func__ << " buffer size=" << buf_len << "less than NDEF msg sise=" <<
        p_mfc->ndef_length;
    return NFC_STATUS_FAILED;
  }
...
static void rw_mfc_process_error() {
  ...
  if (rw_cb.cur_retry < RW_MAX_RETRIES) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << __func__ << "Mifare Classic retransmission attempt "
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << __func__ << "MFC maximum retransmission attempts reached "
      << RW_MAX_RETRIES;
  }
  ...
  if (rw_event == RW_MFC_NDEF_DETECT_EVT) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << __func__ << " status=" << evt_data.status;
