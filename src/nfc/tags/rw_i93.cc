...
void rw_i93_get_product_version(uint8_t* p_uid) {
  ...
  ALOGD("rw_i93_get_product_version ()");
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("product_version = <%s>",
        rw_i93_get_tag_name(p_i93->product_version).c_str());
#else
  ALOGD("product_version = %d", p_i93->product_version);
#endif
...
bool rw_i93_check_sys_info_prot_ext(uint8_t error_code) {
  ...
  ALOGD("rw_i93_check_sys_info_prot_ext ()");
...
void rw_i93_send_to_upper(NFC_HDR* p_resp) {
  ...
  if (event != RW_I93_MAX_EVT) {
    ...
    ALOGE("rw_i93_send_to_upper (): Invalid response");
  }
}
...
bool rw_i93_send_to_lower(NFC_HDR* p_msg) {
  ...
  if (NFC_SendData(NFC_RF_CONN_ID, p_msg) != NFC_STATUS_OK) {
    ALOGE("rw_i93_send_to_lower (): NFC_SendData () failed");
    return false;
  }
...
tNFC_STATUS rw_i93_send_cmd_inventory(uint8_t* p_uid, bool including_afi,
  ...
  ALOGD("rw_i93_send_cmd_inventory () including_afi:%d, AFI:0x%02X",
        including_afi, afi);

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_inventory (): Cannot allocate buffer");
...
tNFC_STATUS rw_i93_send_cmd_stay_quiet(void) {
  ...
  ALOGD("rw_i93_send_cmd_stay_quiet ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_stay_quiet (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_read_single_block(uint16_t block_number,
  ...
  ALOGD("rw_i93_send_cmd_read_single_block ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_read_single_block (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_write_single_block(uint16_t block_number,
  ...
  ALOGD("rw_i93_send_cmd_write_single_block ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_write_single_block (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_lock_block(uint8_t block_number) {
  ...
  ALOGD("rw_i93_send_cmd_lock_block ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_lock_block (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_read_multi_blocks(uint16_t first_block_number,
  ...
  ALOGD("rw_i93_send_cmd_read_multi_blocks ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_read_multi_blocks (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_write_multi_blocks(uint8_t first_block_number,
  ...
  ALOGD("rw_i93_send_cmd_write_multi_blocks ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_write_multi_blocks (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_select(uint8_t* p_uid) {
  ...
  ALOGD("rw_i93_send_cmd_select ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_select (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_reset_to_ready(void) {
  ...
  ALOGD("rw_i93_send_cmd_reset_to_ready ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_reset_to_ready (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_write_afi(uint8_t afi) {
  ...
  ALOGD("rw_i93_send_cmd_write_afi ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_write_afi (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_lock_afi(void) {
  ...
  ALOGD("rw_i93_send_cmd_lock_afi ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_lock_afi (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_write_dsfid(uint8_t dsfid) {
  ...
  ALOGD("rw_i93_send_cmd_write_dsfid ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_write_dsfid (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_lock_dsfid(void) {
  ...
  ALOGD("rw_i93_send_cmd_lock_dsfid ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_lock_dsfid (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_get_sys_info(uint8_t* p_uid, uint8_t extra_flags) {
  ...
  ALOGD("rw_i93_send_cmd_get_sys_info ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_get_sys_info (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_send_cmd_get_multi_block_sec(uint16_t first_block_number,
  ...
  ALOGD("rw_i93_send_cmd_get_multi_block_sec ()");

  p_cmd = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID);

  if (!p_cmd) {
    ALOGE("rw_i93_send_cmd_get_multi_block_sec (): Cannot allocate buffer");
    return NFC_STATUS_NO_BUFFERS;
  }
...
tNFC_STATUS rw_i93_get_next_block_sec(void) {
  ...
  ALOGD("rw_i93_get_next_block_sec ()");

  if (p_i93->num_block <= p_i93->rw_offset) {
    ALOGE("rw_offset(0x%x) must be less than num_block(0x%x)", p_i93->rw_offset,
          p_i93->num_block);
    return NFC_STATUS_FAILED;
  }
...
void rw_i93_sm_detect_ndef(NFC_HDR* p_resp) {
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("rw_i93_sm_detect_ndef () sub_state:%s (0x%x)",
        rw_i93_get_sub_state_name(p_i93->sub_state).c_str(), p_i93->sub_state);
#else
  ALOGD("rw_i93_sm_detect_ndef () sub_state:0x%x", p_i93->sub_state);
#endif
  ...
  if (flags & I93_FLAG_ERROR_DETECTED) {
    if ((length) && (rw_i93_check_sys_info_prot_ext(*p))) {
      ...
      ALOGD("Got error flags (0x%02x)", flags);
  ...
  switch (p_i93->sub_state) {
    case RW_I93_SUBSTATE_WAIT_UID:
      ...
      if (u8 != I93_DFS_UNSUPPORTED) {
        ...
        ALOGD("Got unknown DSFID (0x%02x)", u8);
        rw_i93_handle_error(NFC_STATUS_FAILED);
      } else {
    ...
    case RW_I93_SUBSTATE_WAIT_SYS_INFO:
      ...
      if ((p_i93->block_size == 0) || (p_i93->num_block == 0)) {
        ALOGD("Unable to get tag memory size");
    ...
    case RW_I93_SUBSTATE_WAIT_CC:
      ...
      */
      ...
      ALOGD("rw_i93_sm_detect_ndef (): cc: 0x%02X 0x%02X 0x%02X 0x%02X", cc[0],
            cc[1], cc[2], cc[3]);
      ALOGD("rw_i93_sm_detect_ndef (): Total blocks:0x%04X, Block size:0x%02X",
      ...
      for (xx = 0; xx < length; xx++) {
        ...
        if (p_i93->tlv_detect_state == RW_I93_TLV_DETECT_STATE_TYPE) {
          if (*(p + xx) == I93_ICODE_TLV_TYPE_NULL) {
            ...
            ALOGD("Invalid type: 0x%02x", *(p + xx));
            rw_i93_handle_error(NFC_STATUS_FAILED);
            return;
          }
    ...
    case RW_I93_SUBSTATE_CHECK_LOCK_STATUS:
      ...
      ALOGD("NDEF cur_size(%d),max_size (%d), flags (0x%x)",
...
void rw_i93_sm_read_ndef(NFC_HDR* p_resp) {
  ...
  if (p_i93->rw_length >= p_i93->ndef_length) {
    ...
    ALOGD("NDEF read complete read (%d)/total (%d)", p_resp->len,
          p_i93->ndef_length);

    (*(rw_cb.p_cback))(RW_I93_NDEF_READ_CPLT_EVT, &rw_data);
  } else {
    ALOGD("NDEF read segment read (%d)/total (%d)", p_resp->len,
          p_i93->ndef_length);
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("rw_i93_sm_update_ndef () sub_state:%s (0x%x)",
        rw_i93_get_sub_state_name(p_i93->sub_state).c_str(), p_i93->sub_state);
#else
  ALOGD("rw_i93_sm_update_ndef () sub_state:0x%x", p_i93->sub_state);
#endif
  ...
  switch (p_i93->sub_state) {
    ...
    case RW_I93_SUBSTATE_UPDATE_LEN:
      ...
      if (p_i93->rw_length > 0) {
        ...
        ALOGD("NDEF update complete, %d bytes, (%d-%d)", p_i93->ndef_length,
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("rw_i93_sm_format () sub_state:%s (0x%x)",
        rw_i93_get_sub_state_name(p_i93->sub_state).c_str(), p_i93->sub_state);
#else
  ALOGD("rw_i93_sm_format () sub_state:0x%x", p_i93->sub_state);
#endif
  ...
  switch (p_i93->sub_state) {
    ...
    case RW_I93_SUBSTATE_CHECK_READ_ONLY:
      ...
      if (!p_i93->p_update_data) {
        ALOGE("rw_i93_sm_format (): Cannot allocate buffer");
        rw_i93_handle_error(NFC_STATUS_FAILED);
        break;
      }
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("rw_i93_sm_set_read_only () sub_state:%s (0x%x)",
        rw_i93_get_sub_state_name(p_i93->sub_state).c_str(), p_i93->sub_state);
#else
  ALOGD("rw_i93_sm_set_read_only () sub_state:0x%x", p_i93->sub_state);
#endif
...
void rw_i93_handle_error(tNFC_STATUS status) {
  ...
  ALOGD("rw_i93_handle_error (): status:0x%02X, state:0x%X", status,
        p_i93->state);
...
void rw_i93_process_timeout(TIMER_LIST_ENT* p_tle) {
  ...
  ALOGD("rw_i93_process_timeout () event=%d", p_tle->event);
  ...
  if (p_tle->event == NFC_TTYPE_RW_I93_RESPONSE) {
    if ((rw_cb.tcb.i93.retry_count < RW_MAX_RETRIES) &&
      ...
      ALOGE("rw_i93_process_timeout (): retry_count = %d",
            rw_cb.tcb.i93.retry_count);
    ...
    ALOGE("rw_i93_process_timeout () unknown event=%d", p_tle->event);
  }
}
...
static void rw_i93_data_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  ALOGD("rw_i93_data_cback () event = 0x%X", event);
  ...
  if ((event == NFC_DEACTIVATE_CEVT) || (event == NFC_ERROR_CEVT) ||
    ...
    if (event == NFC_ERROR_CEVT) {
      if ((p_i93->retry_count < RW_MAX_RETRIES) && (p_i93->p_retry_cmd)) {
        ...
        ALOGE("rw_i93_data_cback (): retry_count = %d", p_i93->retry_count);
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("RW I93 state: <%s (%d)>", rw_i93_get_state_name(p_i93->state).c_str(),
        p_i93->state);
#else
  ALOGD("RW I93 state: %d", p_i93->state);
#endif
  ...
  switch (p_i93->state) {
    ...
    default:
      ALOGE("rw_i93_data_cback (): invalid state=%d", p_i93->state);
      GKI_freebuf(p_resp);
      break;
  }
  ...
  if (begin_state != p_i93->state) {
    ALOGD("RW I93 state changed:<%s> -> <%s>",
...
tNFC_STATUS RW_I93Inventory(bool including_afi, uint8_t afi, uint8_t* p_uid) {
  ...
  ALOGD("RW_I93Inventory (), including_afi:%d, AFI:0x%02X", including_afi, afi);

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93Inventory ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
...
tNFC_STATUS RW_I93StayQuiet(void) {
  ...
  ALOGD("RW_I93StayQuiet ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93StayQuiet ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93ReadSingleBlock(uint16_t block_number) {
  ...
  ALOGD("RW_I93ReadSingleBlock () block_number:0x%02X", block_number);

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93ReadSingleBlock ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93WriteSingleBlock(uint16_t block_number, uint8_t* p_data) {
  ...
  ALOGD("RW_I93WriteSingleBlock ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93WriteSingleBlock ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }

  if (rw_cb.tcb.i93.block_size == 0) {
    ALOGE("RW_I93WriteSingleBlock ():Block size is unknown");
    return NFC_STATUS_FAILED;
  }
...
tNFC_STATUS RW_I93LockBlock(uint8_t block_number) {
  ...
  ALOGD("RW_I93LockBlock ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93LockBlock ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93ReadMultipleBlocks(uint16_t first_block_number,
  ...
  ALOGD("RW_I93ReadMultipleBlocks ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93ReadMultipleBlocks ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93WriteMultipleBlocks(uint8_t first_block_number,
  ...
  ALOGD("RW_I93WriteMultipleBlocks ()");
...
tNFC_STATUS RW_I93Select(uint8_t* p_uid) {
  ...
  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93Select ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
  ...
  if (p_uid) {
    ...
    ALOGE("RW_I93Select ():UID shall be provided");
    status = NFC_STATUS_FAILED;
  }
...
tNFC_STATUS RW_I93ResetToReady(void) {
  ...
  ALOGD("RW_I93ResetToReady ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93ResetToReady ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93WriteAFI(uint8_t afi) {
  ...
  ALOGD("RW_I93WriteAFI ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93WriteAFI ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93LockAFI(void) {
  ...
  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93LockAFI ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93WriteDSFID(uint8_t dsfid) {
  ...
  ALOGD("RW_I93WriteDSFID ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93WriteDSFID ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93LockDSFID(void) {
  ...
  ALOGD("RW_I93LockDSFID ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93LockDSFID ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93GetSysInfo(uint8_t* p_uid) {
  ...
  ALOGD("RW_I93GetSysInfo ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93GetSysInfo ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_BUSY;
  }
...
tNFC_STATUS RW_I93GetMultiBlockSecurityStatus(uint16_t first_block_number,
  ...
  ALOGD("RW_I93GetMultiBlockSecurityStatus ()");
...
tNFC_STATUS RW_I93DetectNDef(void) {
  ...
  ALOGD("RW_I93DetectNDef ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93DetectNDef ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_FAILED;
  }
...
tNFC_STATUS RW_I93ReadNDef(void) {
  ALOGD("RW_I93ReadNDef ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93ReadNDef ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_FAILED;
  }
  ...
  if ((rw_cb.tcb.i93.tlv_type == I93_ICODE_TLV_TYPE_NDEF) &&
    ...
    ALOGE("RW_I93ReadNDef ():No NDEF detected");
    return NFC_STATUS_FAILED;
  }
...
tNFC_STATUS RW_I93UpdateNDef(uint16_t length, uint8_t* p_data) {
  ...
  ALOGD("RW_I93UpdateNDef () length:%d", length);

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93UpdateNDef ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_FAILED;
  }
  ...
  if (rw_cb.tcb.i93.tlv_type == I93_ICODE_TLV_TYPE_NDEF) {
    if (rw_cb.tcb.i93.intl_flags & RW_I93_FLAG_READ_ONLY) {
      ALOGE("RW_I93UpdateNDef ():NDEF is read-only");
      return NFC_STATUS_FAILED;
    }
...
tNFC_STATUS RW_I93FormatNDef(void) {
  ...
  ALOGD("RW_I93FormatNDef ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93FormatNDef ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_FAILED;
  }
...
tNFC_STATUS RW_I93SetTagReadOnly(void) {
  ALOGD("RW_I93SetTagReadOnly ()");

  if (rw_cb.tcb.i93.state != RW_I93_STATE_IDLE) {
    ALOGE("RW_I93SetTagReadOnly ():Unable to start command at state (0x%X)",
          rw_cb.tcb.i93.state);
    return NFC_STATUS_FAILED;
  }
  ...
  if (rw_cb.tcb.i93.tlv_type == I93_ICODE_TLV_TYPE_NDEF) {
    if (rw_cb.tcb.i93.intl_flags & RW_I93_FLAG_READ_ONLY) {
      ALOGE("RW_I93SetTagReadOnly ():NDEF is already read-only");
      return NFC_STATUS_FAILED;
    }
    ...
    ALOGE("RW_I93SetTagReadOnly ():No NDEF detected");
    return NFC_STATUS_FAILED;
  }
