...
static bool ce_t4t_send_to_lower(NFC_HDR* p_r_apdu) {
  ...
  if (NFC_SendData(NFC_RF_CONN_ID, p_r_apdu) != NFC_STATUS_OK) {
    ALOGE("ce_t4t_send_to_lower (): NFC_SendData () failed");
    return false;
  }
  return true;
}
...
static bool ce_t4t_send_status(uint16_t status) {
  ...
  ALOGD("ce_t4t_send_status (): Status:0x%04X", status);

  p_r_apdu = (NFC_HDR*)GKI_getpoolbuf(NFC_CE_POOL_ID);

  if (!p_r_apdu) {
    ALOGE("ce_t4t_send_status (): Cannot allocate buffer");
    return false;
  }
...
static bool ce_t4t_select_file(uint16_t file_id) {
  ...
  ALOGD("ce_t4t_select_file (): FileID:0x%04X", file_id);

  if (file_id == T4T_CC_FILE_ID) {
    ALOGD("ce_t4t_select_file (): Select CC file");
  ...
  if (file_id == CE_T4T_MANDATORY_NDEF_FILE_ID) {
    ...
    ALOGE("ce_t4t_select_file (): Cannot find file ID (0x%04X)", file_id);
...
static bool ce_t4t_read_binary(uint16_t offset, uint8_t length) {
  ...
  if (p_src) {
    ...
    if (!p_r_apdu) {
      ALOGE("ce_t4t_read_binary (): Cannot allocate buffer");
      return false;
    }
    ...
    ALOGE("ce_t4t_read_binary (): No selected file");
...
static bool ce_t4t_update_binary(uint16_t offset, uint8_t length,
  ...
  if ((starting_offset == 0) && (p_t4t->nlen > 0)) {
    ...
    if (ce_cb.p_cback) {
      ...
      ALOGD("ce_t4t_update_binary (): Sent CE_T4T_NDEF_UPDATE_CPLT_EVT");
    }
...
static void ce_t4t_set_version_in_cc(uint8_t version) {
  ...
  ALOGD("ce_t4t_set_version_in_cc (): version = 0x%02X", version);
...
static bool ce_t4t_process_select_file_cmd(uint8_t* p_cmd) {
  ...
  ALOGD("ce_t4t_process_select_file_cmd ()");
...
static void ce_t4t_process_select_app_cmd(uint8_t* p_cmd, NFC_HDR* p_c_apdu) {
  ...
  ALOGD("ce_t4t_process_select_app_cmd ()");
  ...
  */
  ...
  if (ce_cb.mem.t4t.selected_aid_idx < CE_T4T_MAX_REG_AID) {
    ...
    if ((*p_cmd) == 0x00) {
      ...
      ALOGD("ce_t4t_process_select_app_cmd (): Not found matched AID");
      status_words = T4T_RSP_NOT_FOUND;
    }
    ...
    ALOGD("CET4T: Forward raw frame (SELECT APP) to wildcard AID handler");
  ...
  if (status_words) {
    ...
    if (status_words == T4T_RSP_CMD_CMPLTED) {
      ...
      ALOGD("ce_t4t_process_select_app_cmd (): T4T CE App selected");
    }
...
void ce_t4t_process_timeout(TIMER_LIST_ENT* p_tle) {
  ...
  ALOGD("ce_t4t_process_timeout () event=%d", p_tle->event);
  ...
  if (p_tle->event == NFC_TTYPE_CE_T4T_UPDATE) {
    ...
    ALOGE("ce_t4t_process_timeout () unknown event=%d", p_tle->event);
  }
}
...
static void ce_t4t_data_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  ALOGD("ce_t4t_data_cback (): conn_id = 0x%02X", conn_id);
  ...
  if (ce_cb.mem.t4t.status & CE_T4T_STATUS_REG_AID_SELECTED) {
    ALOGD("CET4T: Forward raw frame to registered AID");
    ...
    ALOGD("CET4T: Forward raw frame to wildcard AID handler");
    ...
    if (instruct == T4T_CMD_INS_SELECT) {
      ...
      if (select_type == T4T_CMD_P1_SELECT_BY_FILE_ID) {
        ...
        ALOGE("CET4T: Bad P1 byte (0x%02X)", select_type);
      ...
      if ((ce_cb.mem.t4t.status & CE_T4T_STATUS_CC_FILE_SELECTED) ||
        ...
        if ((uint32_t)length <= CE_T4T_MAX_LE) {
          ...
          if (length + offset > max_file_size) {
            if (offset < max_file_size) {
              ...
              ALOGD("CET4T: length is reduced to %d by max_file_size (%d)",
                    length, max_file_size);
            } else {
              ALOGE("CET4T: offset (%d) must be less than max_file_size (%d)",
        ...
        ALOGE("CET4T: File has not been selected");
      ...
      if (ce_cb.mem.t4t.status & CE_T4T_STATUS_NDEF_FILE_READ_ONLY) {
        ...
        ALOGE("CET4T: NDEF File has not been selected");
        ce_t4t_send_status(T4T_RSP_CMD_NOT_ALLOWED);
      }
    } else {
      ALOGE("CET4T: Unsupported Instruction byte (0x%02X)", instruct);
      ce_t4t_send_status(T4T_RSP_INSTR_NOT_SUPPORTED);
    }
  } else {
    ALOGE("CET4T: Application has not been selected");
    ce_t4t_send_status(T4T_RSP_CMD_NOT_ALLOWED);
  }
...
tNFC_STATUS CE_T4tSetLocalNDEFMsg(bool read_only, uint16_t ndef_msg_max,
  ...
  if (!p_ndef_msg) {
    ...
    ALOGD("CE_T4tSetLocalNDEFMsg (): T4T is disabled");
    return NFC_STATUS_OK;
  }
...
tCE_T4T_AID_HANDLE CE_T4tRegisterAID(uint8_t aid_len, uint8_t* p_aid,
  ...
  if (aid_len == 0) {
    ALOGD("CE_T4tRegisterAID (): registering callback for wildcard AID ");
    ...
    ALOGD("CE_T4tRegisterAID (): handle 0x%02x registered (for wildcard AID)",
          CE_T4T_WILDCARD_AID_HANDLE);
    p_t4t->p_wildcard_aid_cback = p_cback;
    return CE_T4T_WILDCARD_AID_HANDLE;
  }

  ALOGD("CE_T4tRegisterAID () AID [%02X%02X%02X%02X...], %d bytes", *p_aid,
        *(p_aid + 1), *(p_aid + 2), *(p_aid + 3), aid_len);

  if (aid_len > NFC_MAX_AID_LEN) {
    ALOGE("CE_T4tRegisterAID (): AID is up to %d bytes", NFC_MAX_AID_LEN);
    return CE_T4T_AID_HANDLE_INVALID;
  }

  if (p_cback == NULL) {
    ALOGE("CE_T4tRegisterAID (): callback must be provided");
    return CE_T4T_AID_HANDLE_INVALID;
  }
  ...
  for (xx = 0; xx < CE_T4T_MAX_REG_AID; xx++) {
    if ((p_t4t->reg_aid[xx].aid_len == aid_len) &&
      ...
      ALOGE("CE_T4tRegisterAID (): already registered");
      return CE_T4T_AID_HANDLE_INVALID;
    }
  }
  ...
  if (xx >= CE_T4T_MAX_REG_AID) {
    ALOGE("CE_T4tRegisterAID (): No resource");
    return CE_T4T_AID_HANDLE_INVALID;
  } else {
    ALOGD("CE_T4tRegisterAID (): handle 0x%02x registered", xx);
  }
...
extern void CE_T4tDeregisterAID(tCE_T4T_AID_HANDLE aid_handle) {
  ...
  ALOGD("CE_T4tDeregisterAID () handle 0x%02x", aid_handle);
  ...
  if (aid_handle == CE_T4T_WILDCARD_AID_HANDLE) {
    if (p_t4t->p_wildcard_aid_cback != NULL) {
      ...
      ALOGE("CE_T4tDeregisterAID (): Invalid handle");
    }
    return;
  }
