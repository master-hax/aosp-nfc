...
static tNFC_STATUS rw_t1t_handle_ndef_rall_rsp(void) {
  ...
  if (p_t1t->work_offset != p_t1t->ndef_msg_len) {
    if ((p_t1t->hr[0] & 0x0F) != 1) {
      ...
      ALOGE("RW_T1tReadNDef - Invalid NDEF len: %u or NDEF corrupted",
            p_t1t->ndef_msg_len);
      status = NFC_STATUS_FAILED;
    }
  } else {
...
tNFC_STATUS RW_T1tFormatNDef(void) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tFormatNDef - Tag not initialized/ Busy! State: %u",
          p_t1t->state);
    return (NFC_STATUS_FAILED);
  }

  if ((p_t1t->hr[0] & 0xF0) != T1T_NDEF_SUPPORTED) {
    ALOGW("RW_T1tFormatNDef - Cannot format tag as NDEF not supported. HR0: %u",
          p_t1t->hr[0]);
  ...
  if (p_ret == NULL) {
    ALOGW("RW_T1tFormatNDef - Invalid HR - HR0: %u, HR1: %u", p_t1t->hr[0],
          p_t1t->hr[1]);
    return (NFC_STATUS_REJECTED);
  }
...
tNFC_STATUS RW_T1tLocateTlv(uint8_t tlv_type) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tLocateTlv - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_FAILED);
  }
  ...
  if ((p_t1t->tlv_detect == TAG_NDEF_TLV) &&
    ...
    ALOGE("RW_T1tLocateTlv - Error: NDEF not supported by the tag");
...
tNFC_STATUS RW_T1tReadNDef(uint8_t* p_buffer, uint16_t buf_len) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tReadNDef - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_FAILED);
  }

  /* Check HR0 if NDEF supported by the tag */
  if (((p_t1t->hr[0]) & 0xF0) != T1T_NDEF_SUPPORTED) {
    ALOGE("RW_T1tReadNDef - Error: NDEF not supported by the tag");
    return (NFC_STATUS_REFUSED);
  }

  if (p_t1t->tag_attribute == RW_T1_TAG_ATTRB_INITIALIZED_NDEF) {
    ALOGW("RW_T1tReadNDef - NDEF Message length is zero, NDEF Length : %u ",
          p_t1t->ndef_msg_len);
  ...
  if (buf_len < p_t1t->ndef_msg_len) {
    ALOGW("RW_T1tReadNDef - buffer size: %u  less than NDEF msg sise: %u",
...
tNFC_STATUS RW_T1tWriteNDef(uint16_t msg_len, uint8_t* p_msg) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tWriteNDef - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_FAILED);
  }

  /* Check HR0 if NDEF supported by the tag */
  if (((p_t1t->hr[0]) & 0xF0) != T1T_NDEF_SUPPORTED) {
    ALOGE("RW_T1tWriteNDef - Error: NDEF not supported by the tag");
    return (NFC_STATUS_REFUSED);
  }

  if ((p_t1t->tag_attribute != RW_T1_TAG_ATTRB_READ_WRITE) &&
      (p_t1t->tag_attribute != RW_T1_TAG_ATTRB_INITIALIZED_NDEF)) {
    ALOGE("RW_T1tWriteNDef - Tag cannot update NDEF");
    return (NFC_STATUS_REFUSED);
  }

  if (msg_len > p_t1t->max_ndef_msg_len) {
    ALOGE("RW_T1tWriteNDef - Cannot write NDEF of size greater than %u bytes",
          p_t1t->max_ndef_msg_len);
    return (NFC_STATUS_REFUSED);
  }
...
tNFC_STATUS RW_T1tSetTagReadOnly(bool b_hard_lock) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tSetTagReadOnly - Busy - State: %u", p_t1t->state);
