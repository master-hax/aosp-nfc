...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("rw_t1t_data_cback (): state:%s (%d)",
        rw_t1t_get_state_name(p_t1t->state).c_str(), p_t1t->state);
#else
  ALOGD("rw_t1t_data_cback (): state=%d", p_t1t->state);
#endif
...
/* Retrasmit the last sent command if retry-count < max retry */
#if (BT_TRACE_VERBOSE == TRUE)
      ALOGE("T1T Frame error. state=%s command (opcode) = 0x%02x",
            rw_t1t_get_state_name(p_t1t->state).c_str(),
            p_cmd_rsp_info->opcode);
#else
      ALOGE("T1T Frame error. state=0x%02x command = 0x%02x ", p_t1t->state,
            p_cmd_rsp_info->opcode);
#endif
  ...
  ALOGD("RW RECV [%s]:0x%x RSP", t1t_info_to_str(p_cmd_rsp_info),
        p_cmd_rsp_info->opcode);
  ...
  if (begin_state != p_t1t->state) {
    ALOGD("RW T1T state changed:<%s> -> <%s>",
...
void rw_t1t_conn_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  ALOGD("rw_t1t_conn_cback: conn_id=%i, evt=0x%x", conn_id, event);
  /* Only handle static conn_id */
  if (conn_id != NFC_RF_CONN_ID) {
    ALOGW("rw_t1t_conn_cback - Not static connection id: =%i", conn_id);
    return;
  }
...
tNFC_STATUS rw_t1t_send_static_cmd(uint8_t opcode, uint8_t add, uint8_t dat) {
  ...
  if (p_cmd_rsp_info) {
    ...
    if (p_data) {
      ...
      ALOGD("RW SENT [%s]:0x%x CMD", t1t_info_to_str(p_cmd_rsp_info),
            p_cmd_rsp_info->opcode);
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("hr0:0x%x, hr1:0x%x", p_t1t->hr[0], p_t1t->hr[1]);
  ALOGD("rw_t1t_handle_rid_rsp (): UID0-3=%02x%02x%02x%02x", p_rid_rsp[0],
        p_rid_rsp[1], p_rid_rsp[2], p_rid_rsp[3]);
#else
  ALOGD("rw_t1t_handle_rid_rsp ()");
#endif
...
tNFC_STATUS rw_t1t_select(uint8_t hr[T1T_HR_LEN],
  ...
  if (p_t1t->p_cur_cmd_buf == NULL) {
    ...
    if (p_t1t->p_cur_cmd_buf == NULL) {
      ALOGE("rw_t1t_select: unable to allocate buffer for retransmission");
      return status;
    }
  }
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGE("T1T timeout. state=%s command (opcode)=0x%02x ",
        rw_t1t_get_state_name(p_t1t->state).c_str(),
        (rw_cb.tcb.t1t.p_cmd_rsp_info)->opcode);
#else
  ALOGE("T1T timeout. state=0x%02x command=0x%02x ", p_t1t->state,
        (rw_cb.tcb.t1t.p_cmd_rsp_info)->opcode);
#endif
...
static void rw_t1t_process_error(void) {
  ...
  ALOGD("rw_t1t_process_error () State: %u", p_t1t->state);
  ...
  if (rw_cb.cur_retry < RW_MAX_RETRIES) {
    ...
    ALOGD("T1T retransmission attempt %i of %i", rw_cb.cur_retry,
          RW_MAX_RETRIES);
    ...
    ALOGD("T1T maximum retransmission attempts reached (%i)", RW_MAX_RETRIES);
    p_t1t->prev_cmd_rsp_info.addr = ((p_cmd_rsp_info->opcode != T1T_CMD_RALL) &&
                                     (p_cmd_rsp_info->opcode != T1T_CMD_RID))
                                        ? p_t1t->addr
                                        : 0;
...
tNFC_STATUS RW_T1tRid(void) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tRid - Busy - State: %u", p_t1t->state);
...
tNFC_STATUS RW_T1tReadAll(void) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tReadAll - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_BUSY);
  }
...
tNFC_STATUS RW_T1tRead(uint8_t block, uint8_t byte) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tRead - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_BUSY);
  }
...
tNFC_STATUS RW_T1tWriteErase(uint8_t block, uint8_t byte, uint8_t new_byte) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tWriteErase - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_BUSY);
  }
  if ((p_t1t->tag_attribute == RW_T1_TAG_ATTRB_READ_ONLY) &&
      (block != T1T_CC_BLOCK) && (byte != T1T_CC_RWA_OFFSET)) {
    ALOGE("RW_T1tWriteErase - Tag is in Read only state");
    return (NFC_STATUS_REFUSED);
  }
  if ((block >= T1T_STATIC_BLOCKS) || (byte >= T1T_BLOCK_SIZE)) {
    ALOGE("RW_T1tWriteErase - Invalid Block/byte: %u / %u", block, byte);
    return (NFC_STATUS_REFUSED);
  }
  if ((block == T1T_UID_BLOCK) || (block == T1T_RES_BLOCK)) {
    ALOGW("RW_T1tWriteErase - Cannot write to Locked block: %u", block);
    return (NFC_STATUS_REFUSED);
  }
...
tNFC_STATUS RW_T1tWriteNoErase(uint8_t block, uint8_t byte, uint8_t new_byte) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tWriteNoErase - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_BUSY);
  }
  if ((p_t1t->tag_attribute == RW_T1_TAG_ATTRB_READ_ONLY) &&
      (block != T1T_CC_BLOCK) && (byte != T1T_CC_RWA_OFFSET)) {
  ...
  if ((block == T1T_UID_BLOCK) || (block == T1T_RES_BLOCK)) {
    ALOGW("RW_T1tWriteNoErase - Cannot write to Locked block: %u", block);
    return (NFC_STATUS_REFUSED);
  }
...
tNFC_STATUS RW_T1tReadSeg(uint8_t segment) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tReadSeg - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_BUSY);
  }
  if (segment >= T1T_MAX_SEGMENTS) {
    ALOGE("RW_T1tReadSeg - Invalid Segment: %u", segment);
    return (NFC_STATUS_REFUSED);
  }
...
tNFC_STATUS RW_T1tRead8(uint8_t block) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tRead8 - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_BUSY);
  }
...
tNFC_STATUS RW_T1tWriteErase8(uint8_t block, uint8_t* p_new_dat) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tWriteErase8 - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_BUSY);
  }

  if ((p_t1t->tag_attribute == RW_T1_TAG_ATTRB_READ_ONLY) &&
      (block != T1T_CC_BLOCK)) {
    ALOGE("RW_T1tWriteErase8 - Tag is in Read only state");
    return (NFC_STATUS_REFUSED);
  }

  if ((block == T1T_UID_BLOCK) || (block == T1T_RES_BLOCK)) {
    ALOGW("RW_T1tWriteErase8 - Cannot write to Locked block: %u", block);
    return (NFC_STATUS_REFUSED);
  }
...
tNFC_STATUS RW_T1tWriteNoErase8(uint8_t block, uint8_t* p_new_dat) {
  ...
  if (p_t1t->state != RW_T1T_STATE_IDLE) {
    ALOGW("RW_T1tWriteNoErase8 - Busy - State: %u", p_t1t->state);
    return (NFC_STATUS_BUSY);
  }

  if ((p_t1t->tag_attribute == RW_T1_TAG_ATTRB_READ_ONLY) &&
      (block != T1T_CC_BLOCK)) {
    ALOGE("RW_T1tWriteNoErase8 - Tag is in Read only state");
    return (NFC_STATUS_REFUSED);
  }

  if ((block == T1T_UID_BLOCK) || (block == T1T_RES_BLOCK)) {
    ALOGW("RW_T1tWriteNoErase8 - Cannot write to Locked block: %u", block);
    return (NFC_STATUS_REFUSED);
  }
