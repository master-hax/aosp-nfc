    ...
    * We may need to change this, when we support multiple version of NFCC */
    ...
    if (nfc_cb.nci_version == NCI_VERSION_2_0) {
      if (evt_data.enable.nci_features & NCI_FEAT_HCI_NETWORK) {
        ...
        ALOGD("hci num_buf=%d buf_size=%d", p_cb->num_buff, p_cb->buff_size);
      } else {
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("nfc_set_state %d (%s)->%d (%s)", nfc_cb.nfc_state,
        nfc_state_name(nfc_cb.nfc_state).c_str(), nfc_state,
        nfc_state_name(nfc_state).c_str());
#else
  ALOGD("nfc_set_state %d->%d", nfc_cb.nfc_state, nfc_state);
#endif
...
void nfc_main_handle_hal_evt(tNFC_HAL_EVT_MSG* p_msg) {
  ...
  ALOGD("nfc_main_handle_hal_evt(): HAL event=0x%x", p_msg->hal_evt);
  ...
  switch (p_msg->hal_evt) {
    ...
    default:
      ALOGE("nfc_main_handle_hal_evt (): unhandled event (0x%x).",
            p_msg->hal_evt);
      break;
  }
}
...
void nfc_main_flush_cmd_queue(void) {
  ...
  ALOGD("nfc_main_flush_cmd_queue ()");
...
void nfc_main_post_hal_evt(uint8_t hal_evt, tHAL_NFC_STATUS status) {
  ...
  if (p_msg != NULL) {
    ...
    ALOGE("nfc_main_post_hal_evt (): No buffer");
  }
}
...
static void nfc_main_hal_cback(uint8_t event, tHAL_NFC_STATUS status) {
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("nfc_main_hal_cback event: %s(0x%x), status=%d",
        nfc_hal_event_name(event).c_str(), event, status);
#else
  ALOGD("nfc_main_hal_cback event: 0x%x, status=%d", event, status);
#endif
  ...
  switch (event) {
    ...
    default:
      ALOGD("nfc_main_hal_cback unhandled event %x", event);
      break;
  }
}
...
static void nfc_main_hal_data_cback(uint16_t data_len, uint8_t* p_data) {
  ...
  if (p_data) {
    ...
    if (p_msg != NULL) {
      ...
      ALOGE("nfc_main_hal_data_cback (): No buffer");
    }
  }
}
...
void NFC_Disable(void) {
  ALOGD("NFC_Disable (): nfc_state = %d", nfc_cb.nfc_state);
...
tNFC_STATUS NFC_DiscoveryMap(uint8_t num, tNFC_DISCOVER_MAPS* p_maps,
  ...
  num_intf = 0;
  ALOGD("nci_interfaces supported by NFCC: 0x%x", nfc_cb.nci_interfaces);
  ...
  for (xx = 0; xx < num_disc_maps; xx++) {
    ...
    if (p_maps[xx].intf_type > NCI_INTERFACE_MAX) {
      ...
      ALOGD("[%d]: vs intf_type:0x%x is_supported:%d", xx, p_maps[xx].intf_type,
            is_supported);
    } else {
      ...
      ALOGD("[%d]: intf_type:%d intf_mask: 0x%x is_supported:%d", xx,
            p_maps[xx].intf_type, intf_mask, is_supported);
    }
    if (is_supported)
      memcpy(&max_maps[num_intf++], &p_maps[xx], sizeof(tNFC_DISCOVER_MAPS));
    else {
      ALOGW("NFC_DiscoveryMap interface=0x%x is not supported by NFCC",
            p_maps[xx].intf_type);
    }
  }
...
tNFC_STATUS NFC_DiscoveryStart(uint8_t num_params,
  ...
  ALOGD("NFC_DiscoveryStart");
  if (nfc_cb.p_disc_pending) {
    ALOGE("There's pending NFC_DiscoveryStart");
  ...
  ALOGD("NFC_DiscoveryStart status: 0x%x", status);
  return status;
}
...
tNFC_STATUS NFC_Deactivate(tNFC_DEACT_TYPE deactivate_type) {
...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("NFC_Deactivate %d (%s) deactivate_type:%d", nfc_cb.nfc_state,
        nfc_state_name(nfc_cb.nfc_state).c_str(), deactivate_type);
#else
  ALOGD("NFC_Deactivate %d deactivate_type:%d", nfc_cb.nfc_state,
        deactivate_type);
#endif
  ...
  if (nfc_cb.nfc_state == NFC_STATE_OPEN) {
    ...
    ALOGD("act_protocol %d credits:%d/%d", p_cb->act_protocol,
          p_cb->init_credits, p_cb->num_buff);
...
tNFC_STATUS NFC_SetPowerOffSleep(bool enable) {
  ALOGD("NFC_SetPowerOffSleep () enable = %d", enable);
  ...
  ALOGE("NFC_SetPowerOffSleep () invalid state = %d", nfc_cb.nfc_state);
...
tNFC_STATUS NFC_PowerCycleNFCC(void) {
  ...
  ALOGE("NFC_PowerCycleNFCC () invalid state = %d", nfc_cb.nfc_state);
  return NFC_STATUS_FAILED;
}
...
void NFC_SetStaticHciCback(tNFC_CONN_CBACK* p_cback) {
  ALOGD("%s dest: %d", __func__, NCI_DEST_TYPE_NFCEE);
