...
#endif
      default:
        ALOGD("nfc_process_quick_timer_evt: unhandled timer event (0x%04x)",
              p_tle->event);
        break;
    }
  }
...
uint32_t nfc_task(uint32_t param) {
  ...
  while (true) {
    ...
    if (event & NFC_TASK_EVT_TRANSPORT_READY) {
      ALOGD("NFC_TASK got NFC_TASK_EVT_TRANSPORT_READY.");
    ...
    if (event & NFC_MBOX_EVT_MASK) {
      ...
      while ((p_msg = (NFC_HDR*)GKI_read_mbox(NFC_MBOX_ID)) != NULL) {
        ...
        switch (p_msg->event & NFC_EVT_MASK) {
          ...
          default:
            ALOGD("nfc_task: unhandle mbox message, event=%04x", p_msg->event);
            break;
        }
