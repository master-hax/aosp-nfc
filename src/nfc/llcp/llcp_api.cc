...
void LLCP_SetTestParams(uint8_t version, uint16_t wks) {
  ALOGD("LLCP_SetTestParams () version:0x%02X, wks:0x%04X", version, wks);
...
void LLCP_SetConfig(uint16_t link_miu, uint8_t opt, uint8_t wt,
  ...
  ALOGD("LLCP_SetConfig () link_miu:%d, opt:0x%02X, wt:%d, link_timeout:%d",
  ...
  ALOGD("                 delay_first_pdu_timeout:%d", delay_first_pdu_timeout);
...
void LLCP_GetConfig(uint16_t* p_link_miu, uint8_t* p_opt, uint8_t* p_wt,
  ...
  *p_delay_first_pdu_timeout = llcp_cb.lcb.delay_first_pdu_timeout;
  ...
  ALOGD("LLCP_GetConfig () link_miu:%d, opt:0x%02X, wt:%d, link_timeout:%d",
  ...
  ALOGD(
      ...
      *p_data_link_timeout);
  ALOGD("                 delay_first_pdu_timeout:%d",
...
tLLCP_STATUS LLCP_ActivateLink(tLLCP_ACTIVATE_CONFIG config,
  ...
  ALOGD("LLCP_ActivateLink () link_state = %d", llcp_cb.lcb.link_state);
...
tLLCP_STATUS LLCP_DeactivateLink(void) {
  ALOGD("LLCP_DeactivateLink () link_state = %d", llcp_cb.lcb.link_state);
...
uint8_t LLCP_RegisterServer(uint8_t reg_sap, uint8_t link_type,
  ...
  ALOGD("LLCP_RegisterServer (): SAP:0x%x, link_type:0x%x, ServiceName:<%s>",
        reg_sap, link_type,
        ((p_service_name.empty()) ? "" : p_service_name.c_str()));

  if (!p_app_cback) {
    ALOGE("LLCP_RegisterServer (): Callback must be provided");
    ...
    ALOGE("LLCP_RegisterServer (): link type (0x%x) must be specified",
          link_type);
    return LLCP_INVALID_SAP;
  }
  ...
  if (reg_sap == LLCP_INVALID_SAP) {
    ...
    if (reg_sap == LLCP_INVALID_SAP) {
      ALOGE("LLCP_RegisterServer (): out of resource");
      return LLCP_INVALID_SAP;
    }
  } else if (reg_sap == LLCP_SAP_LM) {
    ALOGE("LLCP_RegisterServer (): SAP (0x%x) is for link manager", reg_sap);
    return LLCP_INVALID_SAP;
  } else if (reg_sap <= LLCP_UPPER_BOUND_WK_SAP) {
    if (reg_sap >= LLCP_MAX_WKS) {
      ALOGE("LLCP_RegisterServer (): out of resource for SAP (0x%x)", reg_sap);
      return LLCP_INVALID_SAP;
    } else if (llcp_cb.wks_cb[reg_sap].p_app_cback) {
      ALOGE("LLCP_RegisterServer (): SAP (0x%x) is already registered",
            reg_sap);
      return LLCP_INVALID_SAP;
    } else {
    ...
    ALOGE("LLCP_RegisterServer (): SAP (0x%x) must be less than 0x%x", reg_sap,
  ...
  if (!p_service_name.empty()) {
    ...
    if (length > LLCP_MAX_SN_LEN) {
      ALOGE("LLCP_RegisterServer (): Service Name (%d bytes) is too long",
            length);
      return LLCP_INVALID_SAP;
    }
  ...
  ALOGD("LLCP_RegisterServer (): Registered SAP = 0x%02X", reg_sap);
...
uint8_t LLCP_RegisterClient(uint8_t link_type, tLLCP_APP_CBACK* p_app_cback) {
  ...
  ALOGD("LLCP_RegisterClient (): link_type = 0x%x", link_type);

  if (!p_app_cback) {
    ALOGE("LLCP_RegisterClient (): Callback must be provided");
    return LLCP_INVALID_SAP;
  } else if (((link_type & LLCP_LINK_TYPE_LOGICAL_DATA_LINK) == 0x00) &&
             ((link_type & LLCP_LINK_TYPE_DATA_LINK_CONNECTION) == 0x00)) {
    ALOGE("LLCP_RegisterClient (): link type (0x%x) must be specified",
          link_type);
    return LLCP_INVALID_SAP;
  }
  ...
  if (reg_sap == LLCP_INVALID_SAP) {
    ALOGE("LLCP_RegisterClient (): out of resource");
    return LLCP_INVALID_SAP;
  }
  ...
  ALOGD("LLCP_RegisterClient (): Registered SAP = 0x%02X", reg_sap);
...
tLLCP_STATUS LLCP_Deregister(uint8_t local_sap) {
  ...
  ALOGD("LLCP_Deregister () SAP:0x%x", local_sap);

  p_app_cb = llcp_util_get_app_cb(local_sap);

  if ((!p_app_cb) || (p_app_cb->p_app_cback == NULL)) {
    ALOGE("LLCP_Deregister (): SAP (0x%x) is not registered", local_sap);
    return LLCP_STATUS_FAIL;
  }
...
bool LLCP_IsLogicalLinkCongested(uint8_t local_sap, uint8_t num_pending_ui_pdu,
  ...
  ALOGD("LLCP_IsLogicalLinkCongested () Local SAP:0x%x, pending = (%d, %d, %d)",
        local_sap, num_pending_ui_pdu, total_pending_ui_pdu,
        total_pending_i_pdu);
...
tLLCP_STATUS LLCP_SendUI(uint8_t ssap, uint8_t dsap, NFC_HDR* p_buf) {
  ...
  ALOGD("LLCP_SendUI () SSAP=0x%x, DSAP=0x%x", ssap, dsap);

  p_app_cb = llcp_util_get_app_cb(ssap);

  if ((p_app_cb == NULL) || (p_app_cb->p_app_cback == NULL)) {
    ALOGE("LLCP_SendUI (): SSAP (0x%x) is not registered", ssap);
  } else if ((p_app_cb->link_type & LLCP_LINK_TYPE_LOGICAL_DATA_LINK) == 0) {
    ALOGE("LLCP_SendUI (): Logical link on SSAP (0x%x) is not enabled", ssap);
  } else if (llcp_cb.lcb.link_state != LLCP_LINK_STATE_ACTIVATED) {
    ALOGE("LLCP_SendUI (): LLCP link is not activated");
    ...
    if (p_buf->len <= llcp_cb.lcb.peer_miu) {
      if (p_buf->offset >= LLCP_MIN_OFFSET) {
        ...
        ALOGE("LLCP_SendUI (): offset (%d) must be %d at least", p_buf->offset,
              LLCP_MIN_OFFSET);
      }
    } else {
      ALOGE(
    ...
    ALOGE("LLCP_SendUI (): Peer doesn't support connectionless link");
  }
...
bool LLCP_ReadLogicalLinkData(uint8_t local_sap, uint32_t max_data_len,
  ...
  ALOGD("LLCP_ReadLogicalLinkData () Local SAP:0x%x", local_sap);
  ...
  *p_data_len = 0;
  ...
  if ((p_app_cb) && (p_app_cb->p_app_cback)) {
    ...
    ALOGE("LLCP_ReadLogicalLinkData (): Unregistered SAP:0x%x", local_sap);
...
uint32_t LLCP_FlushLogicalLinkRxData(uint8_t local_sap) {
  ...
  ALOGD("LLCP_FlushLogicalLinkRxData () Local SAP:0x%x", local_sap);
  ...
  if ((p_app_cb) && (p_app_cb->p_app_cback)) {
    ...
    ALOGE("LLCP_FlushLogicalLinkRxData (): Unregistered SAP:0x%x", local_sap);
  }
...
tLLCP_STATUS LLCP_ConnectReq(uint8_t reg_sap, uint8_t dsap,
  ...
  ALOGD("LLCP_ConnectReq () reg_sap=0x%x, DSAP=0x%x", reg_sap, dsap);

  if ((llcp_cb.lcb.peer_opt != LLCP_LSC_UNKNOWN) &&
      ((llcp_cb.lcb.peer_opt & LLCP_LSC_2) == 0)) {
    ALOGE("LLCP_ConnectReq (): Peer doesn't support connection-oriented link");
    return LLCP_STATUS_FAIL;
  }
  ...
  /* if application is registered */
  if ((p_app_cb == NULL) || (p_app_cb->p_app_cback == NULL)) {
    ALOGE("LLCP_ConnectReq (): SSAP (0x%x) is not registered", reg_sap);
    return LLCP_STATUS_FAIL;
  }

  if (dsap == LLCP_SAP_LM) {
    ALOGE("LLCP_ConnectReq (): DSAP (0x%x) must not be link manager SAP", dsap);
    return LLCP_STATUS_FAIL;
  }
  ...
  if (p_dlcb) {
    ...
    if (status != LLCP_STATUS_SUCCESS) {
      ALOGE("LLCP_ConnectReq (): Error in state machine");
...
tLLCP_STATUS LLCP_ConnectCfm(uint8_t local_sap, uint8_t remote_sap,
  ...
  ALOGD("LLCP_ConnectCfm () Local SAP:0x%x, Remote SAP:0x%x)", local_sap,
        remote_sap);
  ...
  if (p_dlcb) {
    ...
    ALOGE("LLCP_ConnectCfm (): No data link");
...
tLLCP_STATUS LLCP_ConnectReject(uint8_t local_sap, uint8_t remote_sap,
  ...
  ALOGD("LLCP_ConnectReject () Local SAP:0x%x, Remote SAP:0x%x, reason:0x%x",
  ...
  if (p_dlcb) {
    ...
    ALOGE("LLCP_ConnectReject (): No data link");
    status = LLCP_STATUS_FAIL;
  }
...
tLLCP_STATUS LLCP_SendData(uint8_t local_sap, uint8_t remote_sap,
  ...
  ALOGD("LLCP_SendData () Local SAP:0x%x, Remote SAP:0x%x", local_sap,
        remote_sap);
  ...
  if (p_dlcb) {
    if (p_dlcb->remote_miu >= p_buf->len) {
      if (p_buf->offset >= LLCP_MIN_OFFSET) {
        ...
        ALOGE("LLCP_SendData (): offset (%d) must be %d at least",
    ...
    ALOGE("LLCP_SendData (): No data link");
  }
...
bool LLCP_ReadDataLinkData(uint8_t local_sap, uint8_t remote_sap,
  ...
  ALOGD("LLCP_ReadDataLinkData () Local SAP:0x%x, Remote SAP:0x%x", local_sap,
        remote_sap);
  ...
  *p_data_len = 0;
  if (p_dlcb) {
    ...
    ALOGE("LLCP_ReadDataLinkData (): No data link connection");
...
uint32_t LLCP_FlushDataLinkRxData(uint8_t local_sap, uint8_t remote_sap) {
  ...
  ALOGD("LLCP_FlushDataLinkRxData () Local SAP:0x%x, Remote SAP:0x%x",
  ...
  if (p_dlcb) {
    ...
    ALOGE("LLCP_FlushDataLinkRxData (): No data link connection");
  }
...
tLLCP_STATUS LLCP_DisconnectReq(uint8_t local_sap, uint8_t remote_sap,
  ...
  ALOGD("LLCP_DisconnectReq () Local SAP:0x%x, Remote SAP:0x%x, flush=%d",
  ...
  if (p_dlcb) {
    ...
    ALOGE("LLCP_DisconnectReq (): No data link");
    status = LLCP_STATUS_FAIL;
  }
...
tLLCP_STATUS LLCP_SetTxCompleteNtf(uint8_t local_sap, uint8_t remote_sap) {
  ...
  ALOGD("LLCP_SetTxCompleteNtf () Local SAP:0x%x, Remote SAP:0x%x", local_sap,
        remote_sap);
  ...
  if (p_dlcb) {
    ...
    ALOGE("LLCP_SetTxCompleteNtf (): No data link");
    status = LLCP_STATUS_FAIL;
  }
...
tLLCP_STATUS LLCP_SetLocalBusyStatus(uint8_t local_sap, uint8_t remote_sap,
  ...
  ALOGD("LLCP_SetLocalBusyStatus () Local SAP:0x%x, is_busy=%d", local_sap,
        is_busy);
  ...
  if (p_dlcb) {
    ...
    ALOGE("LLCP_SetLocalBusyStatus (): No data link");
    status = LLCP_STATUS_FAIL;
  }
...
uint16_t LLCP_GetRemoteWKS(void) {
  ALOGD("LLCP_GetRemoteWKS () WKS:0x%04x",
...
uint8_t LLCP_GetRemoteLSC(void) {
  ALOGD("LLCP_GetRemoteLSC () LSC:0x%x",
        (llcp_cb.lcb.link_state == LLCP_LINK_STATE_ACTIVATED)
...
uint8_t LLCP_GetRemoteVersion(void) {
  ALOGD("LLCP_GetRemoteVersion () Version: 0x%x",
        (llcp_cb.lcb.link_state == LLCP_LINK_STATE_ACTIVATED)
...
void LLCP_GetLinkMIU(uint16_t* p_local_link_miu, uint16_t* p_remote_link_miu) {
  ...
  ALOGD("LLCP_GetLinkMIU (): local_link_miu = %d, remote_link_miu = %d",
...
tLLCP_STATUS LLCP_DiscoverService(char* p_name, tLLCP_SDP_CBACK* p_cback,
  ...
  ALOGD("LLCP_DiscoverService () Service Name:%s", p_name);

  if (llcp_cb.lcb.link_state != LLCP_LINK_STATE_ACTIVATED) {
    ALOGE("LLCP_DiscoverService (): Link is not activated");
    return LLCP_STATUS_FAIL;
  }

  if (!p_cback) {
    ALOGE("LLCP_DiscoverService (): Callback must be provided.");
    return LLCP_STATUS_FAIL;
  }
  ...
  if ((llcp_cb.lcb.agreed_major_version == 0x01) &&
    ...
    ALOGE("LLCP_DiscoverService (): Peer doesn't support SNL");
    return LLCP_STATUS_FAIL;
  }
  ...
  ALOGE("LLCP_DiscoverService (): Out of resource");
