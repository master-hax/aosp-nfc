...
#if (BT_TRACE_VERBOSE == TRUE)
  ALOGD("DLC (0x%02X) - state: %s, evt: %s", p_dlcb->local_sap,
        llcp_dlsm_get_state_name(p_dlcb->state).c_str(),
        llcp_dlsm_get_event_name(event).c_str());
#else
  ALOGD("DLC (0x%02X) - state: %d, evt: %d", p_dlcb->local_sap, p_dlcb->state,
        event);
#endif
...
static tLLCP_STATUS llcp_dlsm_idle(tLLCP_DLCB* p_dlcb, tLLCP_DLC_EVENT event,
  ...
  switch (event) {
    ...
    case LLCP_DLC_EVENT_PEER_CONNECT_IND:
      ...
      ALOGD("llcp_dlsm_idle (): Remote MIU:%d, RW:%d", p_dlcb->remote_miu,
            p_dlcb->remote_rw);
    ...
    default:
      ALOGE("llcp_dlsm_idle (): Unexpected event");
      status = LLCP_STATUS_FAIL;
      break;
  }
...
static tLLCP_STATUS llcp_dlsm_w4_remote_resp(tLLCP_DLCB* p_dlcb,
  ...
  switch (event) {
    case LLCP_DLC_EVENT_PEER_CONNECT_CFM:
      ...
      ALOGD("llcp_dlsm_w4_remote_resp (): Remote MIU:%d, RW:%d",
    ...
    default:
      ALOGE("llcp_dlsm_w4_remote_resp (): Unexpected event");
      status = LLCP_STATUS_FAIL;
      break;
  }
...
static tLLCP_STATUS llcp_dlsm_w4_local_resp(tLLCP_DLCB* p_dlcb,
  ...
  switch (event) {
    ...
    default:
      ALOGE("llcp_dlsm_w4_local_resp (): Unexpected event");
      status = LLCP_STATUS_FAIL;
      break;
  }
  ...
  switch (event) {
    ...
    case LLCP_DLC_EVENT_API_DATA_REQ:
      ...
      if (p_dlcb->remote_rw) {
        ...
        ALOGE("llcp_dlsm_connected (): Remote RW is zero: discard data");
    ...
    default:
      ALOGE("llcp_dlsm_connected (): Unexpected event");
      status = LLCP_STATUS_FAIL;
      break;
  }
...
static tLLCP_STATUS llcp_dlsm_w4_remote_dm(tLLCP_DLCB* p_dlcb,
  ...
  switch (event) {
    ...
    default:
      ALOGE("llcp_dlsm_w4_remote_dm (): Unexpected event");
      status = LLCP_STATUS_FAIL;
      break;
  }
...
void llcp_dlc_flush_q(tLLCP_DLCB* p_dlcb) {
  if (p_dlcb) {
    ALOGD("llcp_dlc_flush_q (): local SAP:0x%02X", p_dlcb->local_sap);
    ...
    ALOGE("llcp_dlc_flush_q (): p_dlcb is NULL");
  }
}
...
static void llcp_dlc_proc_connect_pdu(uint8_t dsap, uint8_t ssap,
  ...
  ALOGD("llcp_dlc_proc_connect_pdu ()");
  ...
  if ((p_app_cb == NULL) || (p_app_cb->p_app_cback == NULL) ||
    ...
    ALOGE("llcp_dlc_proc_connect_pdu (): Unregistered SAP:0x%x", dsap);
  ...
  if (llcp_util_parse_connect(p_data, length, &params) != LLCP_STATUS_SUCCESS) {
    ALOGE("llcp_dlc_proc_connect_pdu (): Bad format CONNECT");
    ...
    if (appl_dta_mode_flag == 1 &&
      ...
      ALOGD("%s: Strings are not equal", __func__);
      llcp_util_send_dm(ssap, dsap, LLCP_SAP_DM_REASON_NO_SERVICE);
    } else {
      llcp_util_send_dm(ssap, dsap, LLCP_SAP_DM_REASON_NO_SERVICE);
    }
  ...
  if (dsap == LLCP_SAP_SDP) {
    ...
    if (dsap == LLCP_SAP_SDP) {
      ALOGE("llcp_dlc_proc_connect_pdu (): SDP doesn't accept connection");

      llcp_util_send_dm(ssap, LLCP_SAP_SDP,
                        LLCP_SAP_DM_REASON_PERM_REJECT_THIS);
      return;
    } else if (dsap == 0) {
      ALOGE("llcp_dlc_proc_connect_pdu (): Unregistered Service:%s", params.sn);
  ...
  if (p_dlcb) {
    ALOGE("llcp_dlc_proc_connect_pdu (): Data link is aleady established");
    ...
    if (p_dlcb) {
      ...
      if (status != LLCP_STATUS_SUCCESS) {
        ALOGE("llcp_dlc_proc_connect_pdu (): Error in state machine");
        llcp_util_deallocate_data_link(p_dlcb);
      }
    } else {
      ALOGE("llcp_dlc_proc_connect_pdu (): Out of resource");
...
static void llcp_dlc_proc_disc_pdu(uint8_t dsap, uint8_t ssap, uint16_t length,
  ...
  if (p_dlcb) {
    ...
    ALOGE("llcp_dlc_proc_disc_pdu (): No data link for SAP (0x%x,0x%x)", dsap,
          ssap);
  }
}
...
static void llcp_dlc_proc_cc_pdu(uint8_t dsap, uint8_t ssap, uint16_t length,
  ...
  if (p_dlcb) {
    ...
    if (llcp_util_parse_cc(p_data, length, &(params.miu), &(params.rw)) ==
      ...
      if (status != LLCP_STATUS_SUCCESS) {
        ALOGE("llcp_dlc_proc_cc_pdu (): Error in state machine");
        llcp_util_deallocate_data_link(p_dlcb);
      }
    } else {
      llcp_util_send_frmr(p_dlcb,
    ...
    ALOGE("llcp_dlc_proc_cc_pdu (): No data link for SAP (0x%x,0x%x)", dsap,
          ssap);
  }
}
...
static void llcp_dlc_proc_dm_pdu(uint8_t dsap, uint8_t ssap, uint16_t length,
  ...
  ALOGD("llcp_dlc_proc_dm_pdu ()");

  if (length != LLCP_PDU_DM_SIZE - LLCP_PDU_HEADER_SIZE) {
    ALOGE("llcp_dlc_proc_dm_pdu (): Received invalid DM PDU");
  } else {
    ...
    if (p_dlcb) {
      ...
      ALOGE("llcp_dlc_proc_dm_pdu (): No data link for SAP (0x%x,0x%x)", dsap,
            ssap);
    }
  }
}
...
void llcp_dlc_proc_i_pdu(uint8_t dsap, uint8_t ssap, uint16_t i_pdu_length,
  ...
  if ((p_dlcb) && (p_dlcb->state == LLCP_DLC_STATE_CONNECTED)) {
...
#if (BT_TRACE_VERBOSE == TRUE)
    ALOGD("LLCP RX I PDU - N(S,R):(%d,%d) V(S,SA,R,RA):(%d,%d,%d,%d)", send_seq,
    ...
    if (p_dlcb->next_rx_seq != send_seq) {
      ALOGE("llcp_dlc_proc_i_pdu (): Bad N(S) got:%d, expected:%d", send_seq,
            p_dlcb->next_rx_seq);
      ...
      if ((uint8_t)(send_seq - p_dlcb->sent_ack_seq) % LLCP_SEQ_MODULO >=
          p_dlcb->local_rw) {
        ALOGE("llcp_dlc_proc_i_pdu (): Bad N(S):%d >= V(RA):%d + RW(L):%d",
      ...
      if (!appended) {
        ...
        if (p_msg) {
          ...
          if (p_msg) {
            ...
            ALOGE("llcp_dlc_proc_i_pdu (): out of buffer");
          }
        }
    ...
    ALOGE("llcp_dlc_proc_i_pdu (): No data link for SAP (0x%x,0x%x)", dsap,
          ssap);
...
static void llcp_dlc_proc_rr_rnr_pdu(uint8_t dsap, uint8_t ptype, uint8_t ssap,
  ...
  ALOGD("llcp_dlc_proc_rr_rnr_pdu ()");
  ...
  if (p_dlcb != NULL) {
...
#if (BT_TRACE_VERBOSE == TRUE)
      ALOGD("LLCP RX - N(S,R):(NA,%d) V(S,SA,R,RA):(%d,%d,%d,%d)", rcv_seq,
    ...
    ALOGE("llcp_dlc_proc_rr_rnr_pdu (): No data link for SAP (0x%x,0x%x)", dsap,
          ssap);
  }
}
...
void llcp_dlc_proc_rx_pdu(uint8_t dsap, uint8_t ptype, uint8_t ssap,
  ...
  ALOGD("llcp_dlc_proc_rx_pdu (): DSAP:0x%x, PTYPE:0x%x, SSAP:0x%x", dsap,
        ptype, ssap);

  if (dsap == LLCP_SAP_LM) {
    ALOGE("llcp_dlc_proc_rx_pdu (): Invalid SAP:0x%x for PTYPE:0x%x", dsap,
          ptype);
    return;
  }
  ...
  switch (ptype) {
    ...
    default:
      ALOGE("llcp_dlc_proc_rx_pdu (): Unexpected PDU type (0x%x)", ptype);
...
void llcp_dlc_check_to_send_rr_rnr(void) {
  ...
  ALOGD("llcp_dlc_check_to_send_rr_rnr ()");
...
bool llcp_dlc_is_rw_open(tLLCP_DLCB* p_dlcb) {
  if ((uint8_t)(p_dlcb->next_tx_seq - p_dlcb->rcvd_ack_seq) % LLCP_SEQ_MODULO <
    ...
    ALOGD("llcp_dlc_is_rw_open ():Flow Off, V(S):%d, V(SA):%d, RW(R):%d",
...
#if (BT_TRACE_VERBOSE == TRUE)
      ALOGD("LLCP TX - N(S,R):(%d,%d) V(S,SA,R,RA):(%d,%d,%d,%d)", send_seq,
            p_dlcb->next_rx_seq, p_dlcb->next_tx_seq, p_dlcb->rcvd_ack_seq,
            p_dlcb->next_rx_seq, p_dlcb->sent_ack_seq);
#endif
    } else {
      ALOGE("LLCP - llcp_dlc_get_next_pdu (): offset (%d) must be %d at least",
