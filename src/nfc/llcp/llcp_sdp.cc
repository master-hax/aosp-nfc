...
void llcp_sdp_check_send_snl(void) {
  ...
  if (llcp_cb.sdp_cb.p_snl) {
    ALOGD("SDP: llcp_sdp_check_send_snl ()");
...
tLLCP_STATUS llcp_sdp_send_sdreq(uint8_t tid, char* p_name) {
  ...
  ALOGD("llcp_sdp_send_sdreq (): tid=0x%x, ServiceName=%s", tid, p_name);
...
static tLLCP_STATUS llcp_sdp_send_sdres(uint8_t tid, uint8_t sap) {
  ...
  ALOGD("llcp_sdp_send_sdres (): tid=0x%x, SAP=0x%x", tid, sap);
...
static void llcp_sdp_return_sap(uint8_t tid, uint8_t sap) {
  ...
  ALOGD("llcp_sdp_return_sap (): tid=0x%x, SAP=0x%x", tid, sap);
...
void llcp_sdp_proc_deactivation(void) {
  ...
  ALOGD("llcp_sdp_proc_deactivation ()");
...
tLLCP_STATUS llcp_sdp_proc_snl(uint16_t sdu_length, uint8_t* p) {
    ...
    switch (type) {
      case LLCP_SDREQ_TYPE:
            ...
            (sdu_length >= 2 + length)) /* type, length, TID and service name */
        {
          ...
          if (appl_dta_mode_flag == 1 && sap == 0x00) {
            ALOGD("%s: In dta mode sap == 0x00 p_value = %s", __func__,
                  p_value);
          ...
          if (appl_dta_mode_flag == 1 &&
            ...
            ALOGE("%s: Calling llcp_sdp_send_sdres", __func__);
            tid = 0x01;
            sap = 0x00;
            llcp_sdp_send_sdres(tid, sap);
          }
          ALOGE("llcp_sdp_proc_snl (): bad length (%d) in LLCP_SDREQ_TYPE",
                length);
        }
        break;
      ...
      case LLCP_SDRES_TYPE:
            ...
            && (sdu_length >= 2 + length)) /* type, length, TID and SAP */
        {
          ...
          ALOGE("llcp_sdp_proc_snl (): bad length (%d) in LLCP_SDRES_TYPE",
                length);
        }
        break;

      default:
        ALOGW("llcp_sdp_proc_snl (): Unknown type (0x%x) is ignored", type);
        break;
    }
  ...
  if (sdu_length) {
    ALOGE("llcp_sdp_proc_snl (): Bad format of SNL");
    return LLCP_STATUS_FAIL;
  } else {
