...
tLLCP_STATUS llcp_link_activate(tLLCP_ACTIVATE_CONFIG* p_config) {
  ...
  if (llcp_link_parse_gen_bytes(p_config->gen_bytes_len,
     ...
     * expected In case of wrong magic bytes in ATR_REQ LLC layer will be
     * disconnected but P2P connection is expected to be in connected state.
     * So, non LLC PDU is expected. Below changes is to send PDU after
     * disconnect of LLCP PDU. fix for TC_MAC_TAR_BI_01 LLCP test case */
...
void llcp_link_deactivate(uint8_t reason) {
   ...
   * send DISC PDU to LT. appl_dta_mode_flag condition is added to fulfill
