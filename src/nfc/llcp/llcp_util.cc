  ...
  while (length) {
    ...
    switch (param_type) {
      case LLCP_VERSION_TYPE:
        ...
        ALOGD("Peer Version - 0x%02X", llcp_cb.lcb.peer_version);
        break;
      ...
      case LLCP_MIUX_TYPE:
        ...
        ALOGD("Peer MIU - %d bytes", llcp_cb.lcb.peer_miu);
        break;

      case LLCP_WKS_TYPE:
        BE_STREAM_TO_UINT8(param_len, p);
        BE_STREAM_TO_UINT16(llcp_cb.lcb.peer_wks, p);
        ALOGD("Peer WKS - 0x%04X", llcp_cb.lcb.peer_wks);
        break;
      ...
      case LLCP_LTO_TYPE:
        ...
        ALOGD("Peer LTO - %d ms", llcp_cb.lcb.peer_lto);
        break;

      case LLCP_OPT_TYPE:
        BE_STREAM_TO_UINT8(param_len, p);
        BE_STREAM_TO_UINT8(llcp_cb.lcb.peer_opt, p);
        ALOGD("Peer OPT - 0x%02X", llcp_cb.lcb.peer_opt);
        break;

      default:
        ALOGE("llcp_util_parse_link_params (): Unexpected type 0x%x",
              param_type);
        BE_STREAM_TO_UINT8(param_len, p);
        p += param_len;
        break;
    ...
    if (length >= param_len + 1)
      ...
      ALOGE("llcp_util_parse_link_params (): Bad LTV's");
      return false;
    }
  }
  return true;
  ...
  if (llcp_cb.num_data_link_connection) {
    ...
    for (idx = 0; idx < LLCP_MAX_DATA_LINK; idx++) {
      if (llcp_cb.dlcb[idx].state == LLCP_DLC_STATE_CONNECTED) {
        ...
        ALOGD("DLC[%d], local_rw=%d, rx_congest_threshold=%d", idx,
...
tLLCP_STATUS llcp_util_send_ui(uint8_t ssap, uint8_t dsap,
  ...
  if ((p_app_cb->is_ui_tx_congested) ||
    ...
    ALOGW("Logical link (SAP=0x%X) congested: ui_xmit_q.count=%d", ssap,
...
tLLCP_DLCB* llcp_util_allocate_data_link(uint8_t reg_sap, uint8_t remote_sap) {
  ...
  ALOGD("llcp_util_allocate_data_link (): reg_sap = 0x%x, remote_sap = 0x%x",
        reg_sap, remote_sap);
  ...
  if (!p_dlcb) {
    ALOGE("llcp_util_allocate_data_link (): Out of DLCB");
  } else {
...
void llcp_util_deallocate_data_link(tLLCP_DLCB* p_dlcb) {
  if (p_dlcb) {
    ALOGD("llcp_util_deallocate_data_link (): local_sap = 0x%x",
          p_dlcb->local_sap);
    ...
    if (p_dlcb->state != LLCP_DLC_STATE_IDLE) {
      ...
      ALOGD("llcp_util_deallocate_data_link (): num_data_link_connection = %d",
            llcp_cb.num_data_link_connection);
    }
  }
}
  ...
  while (length) {
    ...
    switch (param_type) {
      case LLCP_MIUX_TYPE:
        ...
        ALOGD("llcp_util_parse_connect (): LLCP_MIUX_TYPE:%d", p_params->miu);
        break;
      ...
      case LLCP_RW_TYPE:
        ...
        ALOGD("llcp_util_parse_connect (): LLCP_RW_TYPE:%d", p_params->rw);
        break;
      ...
      case LLCP_SN_TYPE:
        ...
        ALOGD("llcp_util_parse_connect (): LLCP_SN_TYPE:<%s>", p_params->sn);
        break;

      default:
        ALOGE("llcp_util_parse_connect (): Unexpected type 0x%x", param_type);
        BE_STREAM_TO_UINT8(param_len, p);
        p += param_len;
        break;
    }
    ...
    if (length >= param_len + 1) {
      ...
      ALOGE("llcp_util_parse_connect (): Bad LTV's");
      return LLCP_STATUS_FAIL;
    }
  }
  return LLCP_STATUS_SUCCESS;
  ...
  while (length) {
    ...
    switch (param_type) {
      case LLCP_MIUX_TYPE:
        ...
        ALOGD("llcp_util_parse_cc (): LLCP_MIUX_TYPE:%d", *p_miu);
        break;
      ...
      case LLCP_RW_TYPE:
        ...
        ALOGD("llcp_util_parse_cc (): LLCP_RW_TYPE:%d", *p_rw);
        break;

      default:
        ALOGE("llcp_util_parse_cc (): Unexpected type 0x%x", param_type);
        BE_STREAM_TO_UINT8(param_len, p);
        p += param_len;
        break;
    }
    ...
    if (length >= param_len + 1)
      ...
      ALOGE("llcp_util_parse_cc (): Bad LTV's");
      return LLCP_STATUS_FAIL;
    }
  }
  return LLCP_STATUS_SUCCESS;
...
tLLCP_STATUS llcp_util_send_frmr(tLLCP_DLCB* p_dlcb, uint8_t flags,
  ...
  if (p_msg) {
    ...
    ALOGE("llcp_util_send_frmr (): Out of resource");
    return LLCP_STATUS_FAIL;
  }
}
...
void llcp_util_send_rr_rnr(tLLCP_DLCB* p_dlcb) {
  ...
  if (p_msg) {
    ...
    ALOGD("LLCP TX - N(S,R):(NA,%d) V(S,SA,R,RA):(%d,%d,%d,%d)",
    ...
    ALOGE("llcp_util_send_rr_rnr (): Out of resource");
  }
}
