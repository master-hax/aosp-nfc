...
struct PendingCommand {
    cmd: CommandPacket,
    sndr_rsp: oneshot::Sender<ResponsePacket>,
}

#[derive(Debug)]
struct QueuedCommand {
    pending: PendingCommand,
    sndr_ntf: Option<oneshot::Sender<NotificationPacket>>,
}
...
impl CommandSender {
    ...
    pub async fn send(&mut self, cmd: CommandPacket) -> Result<ResponsePacket> {
        ...
        self.cmd_tx
            .send(QueuedCommand { pending: PendingCommand { cmd, sndr_rsp: tx }, sndr_ntf: None })
            .await?;
        let event = rx.await?;
        Ok(event)
    }
    ...
    pub async fn send_and_notify(
        ...
        self.cmd_tx
            .send(QueuedCommand {
                pending: PendingCommand { cmd, sndr_rsp: tx },
                sndr_ntf: Some(ntx),
            })
            .await?;
        let event = rx.await?;
    ...
    loop {
        select! {
            Some(cmd) = hc.in_cmd_rx.recv() => {
                match cmd.specialize() {
                    Response(rsp) => {
                        ...
                        match pending.take() {
                            Some(PendingCommand{cmd, sndr_rsp}) if cmd.get_op() == this_opcode => {
                                if let Err(e) = sndr_rsp.send(rsp) {
            ...
            Some(queued) = cmd_rx.recv(), if pending.is_none() => {
                if let Some(nsender) = queued.sndr_ntf {
