...
pub async fn init() -> Nci {
    ...
    let ic = InternalChannels { out_dta_int, in_dta_int };
    ...
    Nci { commands, out_dta_ext, in_dta_ext }
}
...
pub struct Nci {
    ...
    pub out_dta_ext: Sender<DataPacket>,
    /// NCI inbound channel for Data messages
    pub in_dta_ext: Receiver<DataPacket>,
}
...
struct CmdRsp {
    cmd: CommandPacket,
    fut: oneshot::Sender<ResponsePacket>,
}

#[derive(Debug)]
struct QueuedCommand {
    cmdrsp: CmdRsp,
    fut: Option<oneshot::Sender<NotificationPacket>>,
}
...
impl CommandSender {
    ...
    pub async fn send(&mut self, cmd: CommandPacket) -> Result<ResponsePacket> {
        ...
        self.cmd_tx.send(QueuedCommand { cmdrsp: CmdRsp { cmd, fut: tx }, fut: None }).await?;
        let event = rx.await?;
        Ok(event)
    }
    /// Send a command which expects notification as a result
    pub async fn send_with_notification(
        &mut self,
        cmd: CommandPacket,
    ) -> Result<(ResponsePacket, oneshot::Receiver<NotificationPacket>)> {
        let (tx, rx) = oneshot::channel::<ResponsePacket>();
        let (ntx, nrx) = oneshot::channel::<NotificationPacket>();
        self.cmd_tx.send(QueuedCommand { cmdrsp: CmdRsp { cmd, fut: tx }, fut: Some(ntx) }).await?;
        let event = rx.await?;
        Ok((event, nrx))
    }
}
...
async fn dispatch(
    ...
    let mut pending: Option<CmdRsp> = None;
    ...
    loop {
        select! {
            Some(cmd) = hc.in_cmd_rx.recv() => {
                match cmd.specialize() {
                    Response(rsp) => {
                        ...
                        match pending.take() {
                            Some(CmdRsp{cmd, fut}) if cmd.get_op() == this_opcode => {
                                if let Err(e) = fut.send(rsp) {
                                    error!("failure dispatching command status {:?}", e);
                                }
                            },
                            Some(CmdRsp{cmd, ..}) => panic!("Waiting for {}, got {}", cmd.get_op(), this_opcode),
            ...
            Some(queued) = cmd_rx.recv(), if pending.is_none() => {
                if let Some(nsender) = queued.fut {
                    ntfs.register(queued.cmdrsp.cmd.get_op(), nsender).await;
                }
                if let Err(e) = hc.out_cmd_tx.send(queued.cmdrsp.cmd.clone().into()) {
                    error!("command queue closed: {:?}", e);
                }
                time_out.as_mut().reset(Instant::now() + Duration::from_millis(20));
                pending = Some(queued.cmdrsp);
            },
            () = &mut time_out => {
