...
pub async fn init(
    ...
    let (out_cmd_tx, in_cmd_rx, out_dta_tx, in_dta_rx) = nfc_hal::init().await;
    ...
    let hc = HalChannels { out_cmd_tx, in_cmd_rx, out_dta_tx, in_dta_rx };
    let ic = InternalCannels { out_cmd_int, in_cmd_int, out_dta_int, in_dta_int };
    tokio::spawn(dispatch(hc, ic));
    (out_cmd_ext, in_cmd_ext, out_dta_ext, in_dta_ext)
}

struct HalChannels {
    out_cmd_tx: UnboundedSender<NciPacket>,
    in_cmd_rx: UnboundedReceiver<NciPacket>,
    out_dta_tx: UnboundedSender<DataPacket>,
    in_dta_rx: UnboundedReceiver<DataPacket>,
}

struct InternalCannels {
...
async fn dispatch(mut hc: HalChannels, mut ic: InternalCannels) -> Result<()> {
