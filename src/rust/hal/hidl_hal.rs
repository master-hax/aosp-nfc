...
pub async fn init() -> Hal {
    ...
    *CALLBACKS.lock().unwrap() = Some(Callbacks {
        ...
        in_dta_tx: inner_hal.in_dta_tx,
    });
    ffi::start_hal();
    hal_open_evt_rx.recv().await.unwrap();

    tokio::spawn(dispatch_outgoing(inner_hal.out_cmd_rx, inner_hal.out_dta_rx));
...
fn on_data(data: &[u8]) {
    ...
    if (data[0] >> 5) & 0x7 != 0 {
        ...
        match DataPacket::parse(data) {
            Ok(p) => callbacks.as_ref().unwrap().in_dta_tx.send(p).unwrap(),
            Err(e) => error!("failure to parse response: {:?} data: {:02x?}", e, data),
        }
    }
}
...
async fn dispatch_outgoing(
    ...
    loop {
        select! {
            ...
            Some(dta) = out_dta_rx.recv() => ffi::send_command(&dta.to_bytes()),
            else => break,
        }
    }
}
