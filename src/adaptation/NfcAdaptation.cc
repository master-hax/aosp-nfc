...
using NfcVendorConfigV1_2 = android::hardware::nfc::V1_2::ExtendedNfcConfig;
...
void NfcAdaptation::GetVendorConfigs(
  ...
  if (mHal_1_2) {
    ...
    mHal_1_1->getConfig([&configValue](NfcVendorConfigV1_1 config) {
      configValue.nfcConfig_1_1 = config;
      configValue.offHostRouteUicc = config.defaultOffHostRoute;
      configValue.offHostRouteEse = 0;
  ...
  if (mHal_1_1 || mHal_1_2) {
    std::vector<uint8_t> nfaPropCfg = {
        configValue.nfcConfig_1_1.nfaProprietaryCfg.protocol18092Active,
        configValue.nfcConfig_1_1.nfaProprietaryCfg.protocolBPrime,
        configValue.nfcConfig_1_1.nfaProprietaryCfg.protocolDual,
        configValue.nfcConfig_1_1.nfaProprietaryCfg.protocol15693,
        configValue.nfcConfig_1_1.nfaProprietaryCfg.protocolKovio,
        configValue.nfcConfig_1_1.nfaProprietaryCfg.protocolMifare,
        configValue.nfcConfig_1_1.nfaProprietaryCfg.discoveryPollKovio,
        configValue.nfcConfig_1_1.nfaProprietaryCfg.discoveryPollBPrime,
        configValue.nfcConfig_1_1.nfaProprietaryCfg.discoveryListenBPrime};
    configMap.emplace(NAME_NFA_PROPRIETARY_CFG, ConfigValue(nfaPropCfg));
    configMap.emplace(
        NAME_NFA_POLL_BAIL_OUT_MODE,
        ConfigValue(configValue.nfcConfig_1_1.nfaPollBailOutMode ? 1 : 0));
    ...
    configMap.emplace(NAME_DEFAULT_ROUTE,
                      ConfigValue(configValue.nfcConfig_1_1.defaultRoute));
    configMap.emplace(
        NAME_DEFAULT_NFCF_ROUTE,
        ConfigValue(configValue.nfcConfig_1_1.defaultOffHostRouteFelica));
    configMap.emplace(NAME_DEFAULT_ISODEP_ROUTE,
                      ConfigValue(configValue.defaultIsoDepRoute));
    configMap.emplace(
        NAME_DEFAULT_SYS_CODE_ROUTE,
        ConfigValue(configValue.nfcConfig_1_1.defaultSystemCodeRoute));
    configMap.emplace(
        NAME_DEFAULT_SYS_CODE_PWR_STATE,
        ConfigValue(configValue.nfcConfig_1_1.defaultSystemCodePowerState));
    configMap.emplace(NAME_OFF_HOST_SIM_PIPE_ID,
                      ConfigValue(configValue.nfcConfig_1_1.offHostSIMPipeId));
    configMap.emplace(NAME_OFF_HOST_ESE_PIPE_ID,
                      ConfigValue(configValue.nfcConfig_1_1.offHostESEPipeId));
    configMap.emplace(
        NAME_ISO_DEP_MAX_TRANSCEIVE,
        ConfigValue(configValue.nfcConfig_1_1.maxIsoDepTransceiveLength));
    if (configValue.nfcConfig_1_1.hostWhitelist.size() != 0) {
      configMap.emplace(NAME_DEVICE_HOST_WHITE_LIST,
                        ConfigValue(configValue.nfcConfig_1_1.hostWhitelist));
    }
    /* For Backwards compatibility */
    if (configValue.nfcConfig_1_1.presenceCheckAlgorithm ==
      ...
      configMap.emplace(
          ...
          ConfigValue(
              (uint32_t)configValue.nfcConfig_1_1.presenceCheckAlgorithm));
    }
  }
}
/*******************************************************************************
