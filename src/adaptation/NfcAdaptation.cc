...
using INfcAidlClientCallback = aidl::android::hardware::nfc::INfcClientCallback;
...
class NfcAidlClientCallback
 ...
 public:
  ...
  ::ndk::ScopedAStatus sendEvent(
      aidl::android::hardware::nfc::NfcEvent event,
      aidl::android::hardware::nfc::NfcStatus event_status) override {
    mEventCallback((uint8_t)event, (tHAL_NFC_STATUS)event_status);
...
NfcAdaptation::NfcAdaptation() {
  ...
  mDeathRecipient = ndk::ScopedAIBinder_DeathRecipient(
      AIBinder_DeathRecipient_new(NfcAdaptation::nfcAidlBinderDied));
}
...
void NfcAdaptation::DeviceShutdown() {
  if (mAidlHal != nullptr) {
    NfcAidlStatus status;
    mAidlHal->closeForPowerOffCase(&status);
...
void NfcAdaptation::HalOpen(tHAL_NFC_CBACK* p_hal_cback,
  ...
  if (mAidlHal != nullptr) {
    ...
    mAidlHal->open(mAidlCallback, &status);
  } else if (mHal_1_1 != nullptr) {
...
void NfcAdaptation::HalCoreInitialized(uint16_t data_len,
  ...
  if (mAidlHal != nullptr) {
    NfcAidlStatus status;
    std::vector<uint8_t> aidl_data(p_core_init_rsp_params,
                                   p_core_init_rsp_params + data_len);
    mAidlHal->coreInitialized(aidl_data, &status);
  } else {
...
void NfcAdaptation::HalControlGranted() {
  ...
  if (mAidlHal != nullptr) {
    NfcAidlStatus status;
    mAidlHal->controlGranted(&status);
  } else {
    mHal->controlGranted();
  }
}
...
void NfcAdaptation::nfcAidlBinderDiedImpl() {
...
// static
void NfcAdaptation::nfcAidlBinderDied(void* cookie) {
  auto thiz = static_cast<NfcAdaptation*>(cookie);
  thiz->nfcAidlBinderDiedImpl();
}
