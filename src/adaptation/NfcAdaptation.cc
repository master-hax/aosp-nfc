...
void NfcAdaptation::Initialize() {
  ...
  LOG(ERROR) << StringPrintf("%s: ver=%s nfa=%s", func, nfca_version_string,
                             nfa_version_string);
  unsigned long num;
...
void NfcAdaptation::HalDeviceContextCallback(nfc_event_t event,
  ...
  DLOG_IF(INFO, appl_trace_level >= BT_TRACE_LEVEL_DEBUG)
      << StringPrintf("%s: event=%u", func, event);
...
void NfcAdaptation::DownloadFirmware() {
  ...
  DLOG_IF(INFO, appl_trace_level >= BT_TRACE_LEVEL_DEBUG)
      << StringPrintf("%s: try open HAL", func);
  HalOpen(HalDownloadFirmwareCallback, HalDownloadFirmwareDataCallback);
  mHalOpenCompletedEvent.wait();

  mHalCloseCompletedEvent.lock();
  DLOG_IF(INFO, appl_trace_level >= BT_TRACE_LEVEL_DEBUG)
      << StringPrintf("%s: try close HAL", func);
  HalClose();
