...
static size_t nfcsnoop_calculate_packet_length(uint16_t type,
                                               const uint8_t* data,
                                               size_t length);

void nfcsnoop_capture(const NFC_HDR* packet, uint64_t timestamp_us) {
  /* TODO: Identify the packet type and check if we need to log this */
  // nfcsnoop_cb();
}

static void nfcsnoop_cb(const uint16_t type, const uint8_t* data,
                        const size_t length, const uint64_t timestamp_us) {
  nfcsnooz_header_t header;

  size_t included_length = nfcsnoop_calculate_packet_length(type, data, length);
  ...
  while (ringbuffer_available(buffer) <
         (included_length + sizeof(nfcsnooz_header_t))) {
...
static size_t nfcsnoop_calculate_packet_length(uint16_t type,
                                               const uint8_t* data,
                                               size_t length) {
  /* TODO: Depending on the type of packet, length should be calculated */
  return 0;
}
...
void nfcif_debug_nfcsnoop_init(void) {
