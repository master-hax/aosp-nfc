...
pub struct Controller {
    ...
    discover_configuration: Vec<nci::DiscoverConfiguration>,
    rf_state: RfState,
    rf_poll_responses: Vec<RfPollResponse>,
}
...
impl Controller {
    ...
    async fn rf_discover(&mut self, cmd: nci::RfDiscoverCommand) -> Result<()> {
        ...
        if self.rf_state != RfState::Idle {
            println!("rf_discover_cmd received in {:?} state", self.rf_state);
        ...
        self.discover_configuration = cmd.get_configurations().clone();
    ...
    async fn rf_discover_select(&mut self, cmd: nci::RfDiscoverSelectCommand) -> Result<()> {
        ...
        if self.rf_state != RfState::WaitForHostSelect {
            println!("rf_discover_select_cmd received in {:?} state", self.rf_state);
        ...
        // STATUS_REJECTED.
        if rf_discovery_id >= self.rf_poll_responses.len() {
        ...
        if cmd.get_rf_protocol() != self.rf_poll_responses[rf_discovery_id].rf_protocol.into() {
    ...
    async fn poll_command(&mut self, cmd: rf::PollCommand) -> Result<()> {
        if self.rf_state != RfState::Discovery {
            return Ok(());
        }
    ...
    async fn nfca_poll_response(&mut self, cmd: rf::NfcAPollResponse) -> Result<()> {
        ...
        for rf_protocol in rf_protocols {
            self.rf_poll_responses.push(RfPollResponse {
                id: cmd.get_sender(),
    ...
    async fn t4at_select_command(&mut self, _cmd: rf::T4ATSelectCommand) -> Result<()> {
        Ok(())
    }

    async fn t4at_select_response(&mut self, cmd: rf::T4ATSelectResponse) -> Result<()> {
        let (rf_discovery_id, rf_interface, rf_protocol) = match self.rf_state {
        ...
        if self.rf_poll_responses[rf_discovery_id].id != cmd.get_sender() {
            return Ok(());
        }

        self.rf_state = RfState::PollActive {
            id: self.rf_poll_responses[rf_discovery_id].id,
            rf_protocol: self.rf_poll_responses[rf_discovery_id].rf_protocol,
            rf_technology: self.rf_poll_responses[rf_discovery_id].rf_technology,
            rf_interface,
        };
        ...
        self.send_control(nci::RfIntfActivatedNotificationBuilder {
            ...
            rf_technology_specific_parameters: self.rf_poll_responses[rf_discovery_id]
    ...
    async fn activate_poll_interface(
        ...
        let rf_technology = self.rf_poll_responses[rf_discovery_id].rf_technology;
        match (rf_interface, rf_technology) {
            (nci::RfInterfaceType::Frame, rf::Technology::NfcA) => {
                self.send_rf(rf::SelectCommandBuilder {
                    sender: self.id,
                    receiver: self.rf_poll_responses[rf_discovery_id].id,
    ...
    pub async fn tick(&mut self) -> Result<()> {
        ...
        for configuration in self.discover_configuration.iter() {
        ...
        if self.rf_state != RfState::Discovery || self.rf_poll_responses.is_empty() {
            return Ok(());
        }
        ...
        if self.rf_poll_responses.len() == 1 {
            let rf_protocol = self.rf_poll_responses[0].rf_protocol.into();
            return self
                .activate_poll_interface(
                    0,
                    rf_protocol,
                    self.select_interface(RfMode::Poll, rf_protocol),
                )
                .await;
        }
        ...
        self.rf_state = RfState::WaitForHostSelect;
        let last_index = self.rf_poll_responses.len() - 1;
        for (index, response) in self.rf_poll_responses.clone().iter().enumerate() {
