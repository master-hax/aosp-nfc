...
impl Controller {
    ...
    async fn rf_deactivate(&self, cmd: nci::RfDeactivateCommand) -> Result<()> {
        ...
        let (status, mut next_state) = match (state.rf_state, cmd.get_deactivation_type()) {
            ...
            (RfState::ListenSleep, Discover) => (nci::Status::Ok, RfState::Discovery),
            ...
            (RfState::WaitForSelectResponse { .. }, _) => {
                (nci::Status::SemanticError, state.rf_state.clone())
            }
        };
        ...
        match next_state {
            ...
            | RfState::WaitForSelectResponse { id, rf_protocol, rf_technology, .. } => {
                self.send_rf(rf::DeselectNotificationBuilder {
                    receiver: id,
                    protocol: rf_protocol,
                    technology: rf_technology,
                    sender: self.id,
                    reason: rf::DeselectReason::DhRequest,
                })
                .await?
            }
            _ => (),
    ...
    async fn receive_data(&self, packet: nci::DataPacket) -> Result<()> {
        println!("+ data({})", packet.get_conn_id());
    ...
    async fn poll_command(&self, cmd: rf::PollCommand) -> Result<()> {
        ...
        if state
            ...
            .iter()
            .find(|config| match (config.technology_and_mode, technology) {
                ...
                | (nci::RfTechnologyAndMode::NfcFPassiveListenMode, rf::Technology::NfcF) => true,
                _ => false,
            })
            .is_some()
        {
    ...
    async fn t4at_select_command(&self, cmd: rf::T4ATSelectCommand) -> Result<()> {
        ...
        // TODO: validate that the protocol and technology are valid for the
        // current discovery settings.

        // TODO: use listen mode routing table to decide which interface
        // should be used for the activating device.
        ...
        self.send_control(nci::RfIntfActivatedNotificationBuilder {
            ...
            // No parameters are currently defined for NFC-A/ISO-DEP Listen Mode.
    ...
    async fn tick(&self) -> Result<()> {
        {
            ...
            for config in state.discover_configuration.iter() {
                self.send_rf(rf::PollCommandBuilder {
                    sender: self.id,
                    receiver: u16::MAX,
                    protocol: rf::Protocol::Undetermined,
                    technology: match config.technology_and_mode {
...

        {
            ...
            if state.rf_poll_responses.len() == 1 {
                ...
                return self
                    .activate_poll_interface(&mut state, 0, rf_protocol, rf_interface)
                    .await;
            }
