...
#  /home/henrichataing/Projects/external/google/pdl/scripts/generate_python_backend.py --output scripts/rf_packets.py
# /!\ Do not edit by hand.
...
class PacketType(enum.IntEnum):
    ...
    SELECT_COMMAND = 0x3
    SELECT_RESPONSE = 0x4
    DESELECT_NOTIFICATION = 0x5

class DeselectReason(enum.IntEnum):
    DH_REQUEST = 0x0
...
class RfPacket(Packet):
    ...
    type: PacketType = field(kw_only=True, default=PacketType.DATA)
    ...
    def parse(span: bytes) -> Tuple['RfPacket', bytes]:
        ...
        try:
            ...
            pass
        try:
            return DeselectNotification.parse(fields.copy(), payload)
        except Exception as exn:
            pass
        try:
...
class PollCommand(RfPacket):
    ...
    def __post_init__(self):
        self.type = PacketType.POLL_COMMAND

    @staticmethod
    def parse(fields: dict, span: bytes) -> Tuple['PollCommand', bytes]:
        if fields['type'] != PacketType.POLL_COMMAND:
...
class NfcAPollResponse(RfPacket):
    ...
    def __post_init__(self):
        ...
        self.type = PacketType.POLL_RESPONSE

    @staticmethod
    def parse(fields: dict, span: bytes) -> Tuple['NfcAPollResponse', bytes]:
        if fields['technology'] != Technology.NFC_A or fields['type'] != PacketType.POLL_RESPONSE:
            raise Exception("Invalid constraint field values")
        if len(span) < 1:
            raise Exception('Invalid packet size')
        nfcid1_size = span[0]
...
class T4ATSelectCommand(RfPacket):
    ...
    def __post_init__(self):
        ...
        self.type = PacketType.SELECT_COMMAND

    @staticmethod
    def parse(fields: dict, span: bytes) -> Tuple['T4ATSelectCommand', bytes]:
        if fields['technology'] != Technology.NFC_A or fields['protocol'] != Protocol.ISO_DEP or fields['type'] != PacketType.SELECT_COMMAND:
            raise Exception("Invalid constraint field values")
        if len(span) < 1:
            raise Exception('Invalid packet size')
        fields['param'] = span[0]
...
class T4ATSelectResponse(RfPacket):
    ...
    def __post_init__(self):
        self.technology = Technology.NFC_A
        self.protocol = Protocol.ISO_DEP
        self.type = PacketType.SELECT_RESPONSE

    @staticmethod
    def parse(fields: dict, span: bytes) -> Tuple['T4ATSelectResponse', bytes]:
        if fields['technology'] != Technology.NFC_A or fields['protocol'] != Protocol.ISO_DEP or fields['type'] != PacketType.SELECT_RESPONSE:
            raise Exception("Invalid constraint field values")
        if len(span) < 1:
            raise Exception('Invalid packet size')
...
class NfcDepSelectCommand(RfPacket):
    ...
    def parse(fields: dict, span: bytes) -> Tuple['NfcDepSelectCommand', bytes]:
        if fields['protocol'] != Protocol.NFC_DEP or fields['type'] != PacketType.SELECT_COMMAND:
            raise Exception("Invalid constraint field values")
        if len(span) < 1:
            raise Exception('Invalid packet size')
...
class NfcDepSelectResponse(RfPacket):
    ...
    def parse(fields: dict, span: bytes) -> Tuple['NfcDepSelectResponse', bytes]:
        if fields['protocol'] != Protocol.NFC_DEP or fields['type'] != PacketType.SELECT_RESPONSE:
            raise Exception("Invalid constraint field values")
        if len(span) < 1:
            raise Exception('Invalid packet size')
...
class SelectCommand(RfPacket):
    ...
    def parse(fields: dict, span: bytes) -> Tuple['SelectCommand', bytes]:
        if fields['type'] != PacketType.SELECT_COMMAND:
            raise Exception("Invalid constraint field values")
...
@dataclass
class DeselectNotification(RfPacket):
    reason: DeselectReason = field(kw_only=True, default=DeselectReason.DH_REQUEST)

    def __post_init__(self):
        self.type = PacketType.DESELECT_NOTIFICATION

    @staticmethod
    def parse(fields: dict, span: bytes) -> Tuple['DeselectNotification', bytes]:
        if fields['type'] != PacketType.DESELECT_NOTIFICATION:
            raise Exception("Invalid constraint field values")
        if len(span) < 1:
            raise Exception('Invalid packet size')
        fields['reason'] = DeselectReason(span[0])
        span = span[1:]
        return DeselectNotification(**fields), span
...
class Data(RfPacket):
    ...
    def parse(fields: dict, span: bytes) -> Tuple['Data', bytes]:
        if fields['type'] != PacketType.DATA:
            raise Exception("Invalid constraint field values")
