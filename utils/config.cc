...
bool ConfigFile::hasValue(const std::string& key) {
...
unsigned ConfigFile::getUnsignedValue(const std::string& key) {
  return getValue(key).getUnsigned();
}

std::vector<uint8_t> ConfigFile::getBytesValue(const std::string& key) {
